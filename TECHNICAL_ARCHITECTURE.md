...
- **MVP scope grew twice more on 2026-08-04**, recorded here for the same reason. First, the offline-map program ([#184](https://github.com/jaimito-asuntos-gringuenos/OurHike/issues/184), vector-first): v1 now builds and hosts its own OpenMapTiles basemap and a corridor DEM archive so the hiking sheet itself works with no signal, rather than shipping only the degraded raster - design and measured costs in [pipeline/BASEMAP.md](pipeline/BASEMAP.md). Second, coverage: the maintainer widened v1 from the AT corridor to the corridor **plus all of New York state** (New Jersey undecided), with the quality bar named as "as good as or better than FarOut" - a clip-shape change for the build, and the reason the multi-trail packaging questions ([#193](https://github.com/jaimito-asuntos-gringuenos/OurHike/issues/193)) stopped being post-MVP abstractions. The 2026-08-04 v2 cap ("map looks good and data is high quality is v1; the broader feature set fast-follows") bounds both.
