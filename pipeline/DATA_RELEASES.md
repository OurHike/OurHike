...
7. **Moved to the standing monitor below — §3a check 6 ([#566](https://github.com/OurHike/OurHike/issues/566)).** `If-Range` is honoured: correct ETag → 206, stale ETag → 200. It is still tested rather than assumed, and still on every published artifact — daily instead of per-release. It left this battery because it is a property of the **bucket** that no release candidate can change, so as a hard gate (RELEASING.md §8 gate 6) it returned the same answer whatever was being released, and every promotion became a promotion over a red gate for a reason nothing in the release could affect. The number is retired rather than reused; 8–19 keep theirs, because this section, `verify_release.py` and several issues all cite them by number.
8. **CORS**: `Access-Control-Expose-Headers` includes `etag`, `content-length`, `content-range`, `accept-ranges` — the exact list in `LAUNCH_CHECKLIST.md:52-60`. A CORS regression silently disarms the `If-Range` comparison on real devices while CI, which is not a browser, would never notice. That is truer since #566 than when it was written: the `If-Range` check now sits in §3a beside the CORS checks, so the same daily run asks both halves.
...
17. Any artifact whose size or feature count drops more than `check_output_quality.DROP_THRESHOLD` (10%) fails, unless `build_state.json` shows the responsible upstream actually changed. This is `flag_drops()` retargeted from run-vs-baseline to release-vs-release — strictly better, because `data/quality_baseline.json` is gitignored and absent on a hosted runner (the vector workflow's own comment admits the check reports SKIPPED there), while the previous release's manifest is always available.
    ...
    **An upstream that changed was the only exemption, and it is the wrong shape for the drops this project actually causes** ([#1143](https://github.com/OurHike/OurHike/issues/1143)). This check has failed 8 of its 9 runs ever, and every one of the five failures in the 2026-08-26 run against production was a *deliberate cull*: the water-reachability gate reaching production (`osm_water` 1,535 → 201 points, #749/#916), an exporter that stopped publishing 72 roads and gaps as resupply (#824), and merged centerline chains (3,025 → 461 features carrying 249,038 of 251,620 vertices, #835). None of those is an upstream change; all of them are somebody's decision, and until now there was no way to record one — so the gate stayed red and the verdict lived nowhere.

    **The stored size is not the data, and until [#1173](https://github.com/OurHike/OurHike/issues/1173) this check could not tell the difference.** It sizes each artifact with a HEAD, and R2 stores what was uploaded — so for the gzipped text artifacts that Content-Length is the *compressed* size, and a build that compresses better reads as one that lost data. A change of storage *format* was already handled (the encodings are compared before the sizes are, #717); a change in how well the same format compresses the same bytes was not. So both figures `latest.json` publishes are now compared: `size_bytes` decoded and `transfer_bytes` on the wire. A drop past the threshold **on the wire alone is a WARN** naming the compression ratio before and after, and does not gate — `--strict` does not escalate it either, because a WARN is a check that ran and answered, where a SKIP is one that could not run. A drop in **both** fails exactly as before. Where either release publishes no `size_bytes`, the stored drop stands on its own and fails: "cannot tell" must not read as "did not drop".

    **What that still does not settle**, recorded because the run that prompted it is still red under it: v1.2.0's `nearby_trails.geojson` fell 23,469,839 → 15,921,393 decoded (−32%) as well as 7,703,741 → 3,729,336 stored (−52%), because [#1113](https://github.com/OurHike/OurHike/issues/1113) capped coordinates at six decimals and stopped writing about eleven junk digits per number — with 21,805 features and all five organizations still present. Both figures are past 10%, so the check still fails it, and no byte count can tell that apart from real loss. Only a feature count can, which is #1173's option 3 and needs a count published beside each artifact. A precision cap is meanwhile exactly the shape `acknowledged_drops.json` below exists to sign for.

    `reference/acknowledged_drops.json` is that record, and two properties keep it from becoming a place checks go to die. **An entry names the predecessor its drop was measured against**, so it applies to exactly one pair of releases and is inert the moment a newer release takes that place — an acknowledgement cannot outlive the comparison it was written for. And **an entry is a ceiling, not a blanket**: a drop past its `max_drop` fails exactly as before, saying that a row exists and does not cover this much. Every row carries who decided it and the measurement that establishes it is not data loss, because a row without evidence turns a safety gate off on somebody's say-so.
...
22. **Launch budget** ([#1254](https://github.com/OurHike/OurHike/issues/1254)): every artifact the client fetches whole — everything `publish.py` gzips, which is everything not read by byte range (`.pmtiles`, `.fgb`) — decodes to no more than `LAUNCH_ARTIFACT_BUDGET_BYTES` in `client/src/lib/artifactBudget.ts`, read out of the client rather than restated, the way check 2 reads its keys. The client declines a bigger artifact before fetching it; this check is what stops one being promoted at all. A text artifact with no `size_bytes` is a skip, never a pass. Written the day gate 6 passed a 228.8 MB `nearby_trails.geojson` and a 78.6 MB `trail_graph.json` to production — the first crashed the map, the second froze the first page — with check 15 having looked straight at the cause ([#1231](https://github.com/OurHike/OurHike/issues/1231)) and recorded it as an exception. Since [#1257](https://github.com/OurHike/OurHike/issues/1257) stage 3 those two files, and the graph's companions, stay in the bucket without any current client fetching them whole — the lines are tiles and the graph is cells — so the check weighs only what the client declares in `config.ts` plus every graph cell shard by its shape (`trail_graph[_<half>]_cell_<name>.json`, derived from the index rather than declared), and skips a whole file by name with the reason rather than failing a release for a file no phone parses.
...
6. **`If-Range` is honoured** — a current ETag is answered `206` and the transfer continues; a **stale** one is answered `200` and the range ignored, so held bytes are discarded rather than spliced onto new ones. This was §3's check 7 until [#566](https://github.com/OurHike/OurHike/issues/566) and belongs here for the reason §3 now records: it is a fact about the bucket, and a release gate that answers the same regardless of the candidate carries no information about it. **Known failing** on both `r2.dev` and the `data.ourhike.org` custom domain (measured 2026-08-15; every response `cf-cache-status: DYNAMIC`, so the cache that would have arbitrated never enters the path) — a green here means the endpoint changed, which is exactly the day a daily monitor exists to catch and a waved-through gate would not have.

   **It is reported, not alarmed on.** Unlike every other check here it declares itself `hiker_facing: False`, so it never contributes to *"a hiker cannot download the map"*. `archiveDownload.ts` performs the same comparison against the ETag the `206` carries, with the published SHA-256 behind that, so the absent half is the server's belt and not the client's braces. A monitor that reported an outage every morning over a known-absent belt is one nobody would be reading on the morning the braces went too.
...
**And once per run, one check that is not about bytes** ([#916](https://github.com/OurHike/OurHike/issues/916)): every published `osm_water` point measured against the trail, side trails, shelters and campsites the same bucket serves, and required inside the 100 ft gate [#749](https://github.com/OurHike/OurHike/issues/749) applies at build time. It is here because of *where* the failure it catches lives. On 2026-08-21 the bucket was serving 1,535 ungated points — 1,159 past five miles — with #749's gate working, the run that applied it green, and production's root keys simply older than the gate. Nothing in a build's own output could have said so; only something reading the bucket could. It costs nothing: all four artifacts it needs are already pulled in full by the hash check, so their bytes are kept as they stream rather than fetched again, and only when the hash matched. It is measured against the *published* trails layer, which `export_trails.py` has simplified, so it carries that simplification's own one-metre bound as its tolerance — a property of the geometry, not slack anybody chose.
