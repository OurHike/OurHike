...
- **Data Saver is on.** `effectiveBackground()` subtracts the whole live sheet, and for someone who has downloaded nothing there is no archive underneath, so the override *is* the entire map. Settings said so; the map screen did not.
