...
| [WEATHER.md](features/WEATHER.md) | **v2, twelfth feature — designed, spike run, not built.** A forecast for every trail the map draws, published hourly to R2 from NOAA's own model grid rather than asked of a per-point API — 142,299 miles of trail is ~74,330 forecast grid squares, which no free API serves and several forbid republishing. Measured, not argued: correcting the forecast for elevation made it worse, and a day-old forecast is only ~0.2 °F worse on temperature, so what goes stale out of signal is warnings and storm timing. Takes the forecast half of HIKER_SAFETY.md §3–4. |
...
## v2 — weather along every trail

**Scoped 2026-09-24 as v2's twelfth feature: [features/WEATHER.md](features/WEATHER.md), tracked as [#1056](https://github.com/OurHike/OurHike/issues/1056) — *v2: weather on Today, on a waypoint, and along a planned route*.** The first feature that tells a hiker about something the map cannot hold still: storms, cold, and the next few days at the place they will sleep.

The maintainer set its shape by poll — every trail rather than only the A.T., a scheduled publish to R2, sampled along each trail, refreshed about hourly — and the doc answers each against measurements:

- **The source is decided by volume before licence.** 142,299 miles of trail sits under ~74,330 2.5 km forecast grid squares, ~1.8 million lookups a day at one per square per hour. That rules out every per-point API, Pirate Weather included, and leaves NOAA's National Blend of Models downloaded whole from its public-domain AWS bucket.
- **Finer than the grid is not more precise.** Correcting NBM's temperature for elevation made tomorrow's high worse, 2.7 °F off on average across eight stations becoming 3.0 °F, and 2.8 becoming 4.5 at Mount Washington — measured over 61 summer days, with winter still to check.
- **What goes stale out of signal is warnings, not temperature.** A day-old forecast of tomorrow's high is ~0.2 °F worse than a fresh one; a warning issued after the phone's last contact is simply absent. So after hours without signal the card keeps its days and says "no word on warnings since", never "no warnings".
- **An hourly publish needs an hourly clock, and GitHub's is not one** — #1346 measured the "hourly" conditions bake at every 3 h 53 m. Where the clock comes from is the maintainer's open question.
