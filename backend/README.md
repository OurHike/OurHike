...
**The image builds and runs, measured 2026-09-09 (#600)** - built unmodified and run against a real Postgres, `PORT` honoured at a non-default value and at its 8000 fallback, `/health` 200, no tables created by booting, and 16 tables with RLS on all 16 after `alembic upgrade head` in the container. `../LAUNCH_CHECKLIST.md` 6 carries the full result and, more usefully, what it still does not cover: a direct connection rather than Supabase's transaction pooler (#95), and no real Supabase project. The container also runs as **root** - no `USER` in the `Dockerfile`.
