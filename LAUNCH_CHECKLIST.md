...
**The order 4.3 to 4.5 run in is the point.** Run this check with `AUTH_PROVIDERS` still at its old value and every 4.3 step done; only when it passes set the variable to `google,github,email` and run it once more. Set early, the variable is #397's defect again with two buttons.
