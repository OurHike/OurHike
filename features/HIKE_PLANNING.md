...
- **Two of the design's nine label classes have no data**, and are absent rather than shipped inert: `pipeline/lib/poi_schema.py` publishes no `trailhead`, and junctions exist only as graph nodes in `build_trail_graph.py`. Tier 1 of the ladder is "trailheads, parking, road names" and two thirds of it can be answered.
...
- **There is still no elevation profile on this screen, and there cannot honestly be one yet.** The design's rail carries a silhouette; `pipeline/export_network_elevation.py` publishes `[gain_ft, loss_ft]` per edge and says why — dense samples are "worth publishing only if a chart is ever drawn for a network route, and then as a fourth artifact fetched when that chart opens." A silhouette drawn from two scalars per edge would be a shape nobody measured. The panel prints the climb figures that *are* published and one line saying the shape is not.
