...
| `pr-preview.yml` | The automatic build on every push stays on production's data — that is what most reviewers should see. A `workflow_dispatch` run, dispatched from the pull request's own branch with a `data_environment` choice (`pr_number` required alongside it, since the run carries no pull request context to read one from), points that same build at `environments/ua` instead — the same formula as `ua.yml`. This is how a pipeline change gets looked at end to end before it reaches production: publish it to UA (the dispatch instruction in [CLAUDE.md](../CLAUDE.md)), then dispatch this workflow the same way against the pull request that changed it. **One fallback, since 2026-09-10 ([#1374](https://github.com/OurHike/OurHike/pull/1374)):** where the branch pins `DATA_RELEASE` to an id production does not carry yet and UA does, the automatic build previews UA and the comment says so — the alternative was a preview that 404s its own manifest, which the maintainer met on a phone as a first-run place field that would not enable. Production stays the default whenever it can serve the pin |
