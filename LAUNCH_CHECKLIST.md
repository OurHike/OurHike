...
**⚠️ The live bucket needs this re-pasted, once — this is measured, not predicted.** Asked directly on 2026-08-09, `pub-31203373f21e449194a97b681bc24b91.r2.dev` answers a preflight for `if-match` with `204` and one for **`if-range` with `403 Forbidden`**. The four origins are all allowed and all four expose-headers are readable, so first downloads are fine; it is **resuming** an interrupted archive that the browser refuses today. Run the command above and paste the result over the existing policy. Nothing else about the bucket changes, and the daily **Check deployment** workflow will keep saying so until it is done.
