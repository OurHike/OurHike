...
**Only water sources, shelters/campsites, resupply points and — since #1122 — parking** get this treatment, per "just important things like food and shelter". Still explicitly not viewpoints, bridges, or privies. Keeps the ask limited to logistics/safety-relevant data, the same prioritization this project has applied elsewhere (e.g. Trail Blaze Colors focusing on safety-relevant accuracy, not every cosmetic detail).

**Parking was on the excluded list until 2026-08-27**, and this paragraph named it there. What changed is not the principle but which side of it parking falls on, and the argument was already written down for a different purpose: `client/src/map/poiPriority.ts` ranks parking above the rest of the tail "because it is the way off the trail: the pin a hiker looks for when the weather turns or an ankle goes, which is the same argument water and shelter win on." A full lot at a trailhead is a hiker's exit being unavailable. It is also the one place on this map where **capacity is a fact somebody standing there can see** — which is exactly what shelters' `full` was not, and why the two moved in opposite directions in the same change.

The list has three readers beyond the card, and every one of them moves with it: `lib/stalenessDisplay.ts` (which pins wear a freshness tier), `lib/passedToday.ts` (the Volunteer tab's evening list, which now includes trailhead lots), and `chrome/PoiCard.tsx` (whether the expand promises notes).
...
- **Overnight conditions.** A stale shelter/campsite pin offers the same quick-tap pattern plus an optional note. **The answers are `fine` / `problem` / `trash`, not the "available / crowded / full" this line sketched (#1122; `problem` was `damaged` until #1140, where the word was found to be narrower than the thumbs-down it stood for).** Capacity is the number this project has already decided it will not claim — a shelter whose capacity nobody stands behind exports no capacity, because "a hiker deciding whether to push on to the next shelter is better served by no answer than by a made-up one" (CLAUDE.md) — and a one-tap `full` is that same unbacked figure wearing a button, arriving dated and attributed, which is worse than the export's silence rather than better. **A real integration worth having, not fully designed here:** once Segments/Trip Planning exist, marking a day's Segment complete at a shelter is the same moment a hiker would naturally see this prompt - combining them into one interaction serves "don't want to spend a ton of time on their phone" directly, rather than asking twice.
...
A "water's flowing fine" or "shelter was fine" check-in is a **confirmation**, not a problem report - routing it through the same moderation queue Report a Problem uses for accusations/hazards would be real overhead for no reason (there's nothing to verify about "someone confirmed normal conditions"). So it's a separate, lighter model that writes directly to the POI's `last_confirmed_at` with no moderation step. **It can still escalate into a real `Report`** when the quick answer actually indicates a problem (tapping "dry" prompts "want to report this?"; `trash` goes straight to the report type that names it, and `problem` opens the picker for the same reason `dry` does — #1140 took that shortcut away when the button stopped promising structural damage) - reusing Report a Problem's machinery exactly where it's actually needed, rather than duplicating problem-handling logic here too.
...
- **Two more surfaces, both places the hiker already looks.** A stale water source ahead carries the same tier styling in the next-up rail beside the elevation ribbon (`client/src/chrome/NextUpRail.tsx`) — the most-looked-at strip in the app. This was written as owed work costing "a `match` expression because the lane is already drawn"; on the rail it is **built**, which is where [#1054](https://github.com/OurHike/OurHike/issues/1054) left it: the rail takes a `stalenessFor` prop and draws the treatments from `lib/stalenessDisplay.ts`, the one policy home, rather than a fourth copy of the tiers. And a "places you passed today" list in the Volunteer tab, for logging from memory at camp — still owed.
