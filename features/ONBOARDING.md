...
- **A brief welcome/value-prop moment** - what OurHike is, and that the organizations who keep these trails open take members and donations *directly* while OurHike takes no cut and holds no money, skippable in one tap. Not a multi-page pitch. **This read "that it funds the ATC and affiliated clubs" until 2026-08-27**, and it was never true: no money passes from OurHike to any organization and there is no arrangement under which it would. The maintainer, that day: *"There is no funding model today for the orgs. The hope is we will drive membership and donations to those orgs."* Two other places spell this same screen and all three have to say one thing - [WIREFRAMES.md](../WIREFRAMES.md) §5 screen 1, and the shipped copy in `client/src/screens/Onboarding.tsx`'s `what-ourhike-is` step, which is guarded by a test asserting that nothing on that step puts OurHike, a purchase or a donation in front of the verb "fund" in any inflection. It is a guard against this sentence coming back, not a proof that no screen anywhere can say it - it reads the value-prop step only.
