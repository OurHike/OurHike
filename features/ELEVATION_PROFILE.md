...
## The lanes go where the ribbon goes (#913)

The bullet above drew the three waypoint lanes under the fix window and under nothing else. The number in it is not in dispute: a pill swallows `COLLAPSE_THRESHOLD_PCT` — 1.5% — of whatever window it is drawn in, so over a 60-mile plan it stands for 0.9 mi of trail, and that really is why the fix window is ten miles rather than forty.

What it was weighed against is the part that does not survive contact with the other three domains. On the fix window the alternative to individual pins is *a ten-mile lane that reads well*, which is worth protecting. On a planned stretch, or on the piece of trail somebody has just panned to, the alternative is **an empty strip under the profile** — and a hiker laying out an evening reads that as *there is nothing along here*. That is a claim about water, made by silence, on the surface where somebody is deciding how far to walk before they can drink. A pill saying `3 water` over nine tenths of a mile is coarse **and true**, and the pill is the wireframe's own answer to crowding (WIREFRAMES.md §1.4) rather than a device invented for this screen.

So `ribbonLanes` (`lib/ribbonView.ts`) builds the lanes from the same decision that picks the ribbon, and `MapScreen` gets one window for both. Four things hold it up:

- **Which mile places a pin depends on which domain won, and that rule lives beside the domains.** `ahead` is windowed on the *client index's* axis — the fault this page already carries, one section up — and its lanes have always been placed from the same index, so nothing moves under a walking hiker. Every other domain is a pipeline-axis span, so its pins come from the POI's own published `mile` (#753). Crossing the two puts a pin a couple of tenths off the climb it sits under, which is [HIKE_PLANNING.md](HIKE_PLANNING.md) Finding 1 drawn twice on one screen. Deciding it in the shell would have put the trap in the caller.
- **The window is the view's `domain`, not its samples' ends.** Not academic: the profile is sampled every 25 m, so the last sample at or before a domain's end falls short of it — far enough that windowing on samples **dropped the stop a route was walking to out of the SLEEP lane**, which is how the bug was found. `ElevationRibbon` now takes that domain explicitly (defaulting to what it always did), so the profile is drawn at its real fraction of the width and a domain whose DEM coverage starts late draws a line that starts late instead of one stretched over ground nothing measured.
- **Refused rather than emptied, in the cases that would lie.** A POI set with no usable mile in it at all — a download published before #753, or a centerline index that placed nothing — gets no lanes, because empty ones would report "nothing along here" about POIs the app cannot place anywhere. A set that *has* miles and genuinely holds nothing in the window draws empty lanes, because that emptiness is a fact about the trail.
- **A span ceiling, derived rather than picked.** `MAX_LANE_SPAN_MI` is `8 / 0.015` ≈ **533 miles**: eight miles is the shelter spacing Decision 1 already rests its look-ahead on, and where a pill swallows that much ground a lane has stopped naming places and started drawing density. This is what keeps `whole-trail` — 1.5% of the AT is **33 miles** — from wearing three rows of pills, and it is why the resting ribbon has no lanes under it while a plan or a map view does.

**What that ceiling is and is not.** It is **reasoned, not measured**, and eight miles is the *only* lane spacing this repository has a figure for. Water — the lane a hiker planning an evening is likeliest to be reading — has no census here at all, and the published counts in [pipeline/README.md](../pipeline/README.md) point to it being sparser than the shelters rather than denser, which would make a tighter bound the right one. So the number marks where the lanes stop being lanes, not where they stop reading well. A census off `export_poi.py`'s `attach_miles` would settle the water-shaped bound.
