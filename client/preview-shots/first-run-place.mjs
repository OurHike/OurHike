...
export const caption =
  'First run, card 3 of 5 — "Where do you hike?", a place the hiker names, kept with their settings so a second device opens on it too — never a location fix (#1373, frame 1b; synced since #1374)'
export const alt =
  'The third first-run card over the hero photo: "Where do you hike?" with the sentence "So we can put a trail in front of you before you turn location on — and fall back to it whenever GPS cannot get a fix", a search field placeholder reading Town, park, or trailhead with "harr" typed into it and, where the places index is on the bucket, rows such as Harriman State Park, NY with the miles of trail held; under the field, "Kept with your settings — on this phone, and with your account once you sign in, so another device opens here too. Change it any time in More → You." and a Skip link'
...
export default async function drive(page) {
  ...
  // Past the mode card (the review of #1374) the way the design's own
  // walkthrough goes: a mode taken, then on. Day hike is the one the skip
  // would have meant anyway.
  await page.getByRole('radio', { name: /^Day hike/ }).click()
  await page.getByRole('button', { name: 'Continue as day hike' }).click()
