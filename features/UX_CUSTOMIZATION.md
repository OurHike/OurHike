...
Companion to [FEATURES.md](../FEATURES.md), [TECHNICAL_ARCHITECTURE.md](../TECHNICAL_ARCHITECTURE.md), and [OurHikeValues.md](../OurHikeValues.md). Extends [MAP_OPTIONS.md](MAP_OPTIONS.md)'s already-designed MapLibre chrome; auto-rotate was designed to reuse the same GPS-trajectory bearing math the wrong-way alert's design called for, and neither one was ever built ([HIKER_SAFETY.md](HIKER_SAFETY.md) §5). [ONBOARDING.md](ONBOARDING.md) is where these settings first get a one-line mention - deliberately not a walkthrough of every default here. Data model consolidated 2026-07-28 into [IDENTITY_AND_PRIVACY.md](IDENTITY_AND_PRIVACY.md)'s `UserPreferences`.
...
**Compass sensors are also just noisy in practice, worth naming honestly:** magnetometer readings jitter, and hiking gear (trekking poles, a stove, anything metal near the phone) can throw off a raw compass reading. **Design recommendation: prefer GPS-derived movement bearing over the device compass while actually walking, falling back to the compass only when stationary** (GPS bearing is meaningless if you're not moving). This was designed to reuse the wrong-way alert's bearing computation directly - the same trailing-GPS-window trajectory math, not a second implementation - but neither one was ever built, and auto-rotate is in the same unbuilt state ([HIKER_SAFETY.md](HIKER_SAFETY.md) §5). Once that math exists, it would still sidestep most of the iOS/Capacitor compass complexity above for the common case of a hiker actually walking.
...
**Designed 2026-08-12 - the waypoint-type half has a doc of its own, [POI_VISIBILITY.md](POI_VISIBILITY.md), and this section stays its `why`.** The two questions above are still two questions and the split still holds; what that doc adds is a third the split did not anticipate. Which types a hiker *asks* for is only half of what reaches the screen - the map draws a fraction of what it holds at every zoom, silently, and the persistent filter this section describes turns out to be the main lever a hiker has over which fraction. That is what moves `waypoint_types_shown` from a tidy preference to something worth building: hiding a category hands its space to the ones left. The open question below about its default is unchanged and gains weight there.

**Built 2026-08-07 - `layer_detail_level` is the second of the five to ship, wired per MAP_STYLE_SPEC.md's matrix.** The built form is per-layer *visibility tiers* on the live sheet rather than the zoom-threshold density sketched above: `full` draws everything, `standard` (the default) drops only admin borders, and `minimal` keeps terrain and navigation - index contours, side paths, peaks, places, water, major roads - while dropping minor contours and their labels, tracks, minor roads, water labels and the scrub overprint. The matrix lives in `client/src/map/mapDetail.ts`, applied as pure `visibility` writes on the live map (never a rebuild - the same rule every map preference keeps), and the control is a segmented picker in Settings (`client/src/screens/MapDetailPicker.tsx`). The downloaded raster background has no layers to thin and ignores it. Waypoint-type display remains unbuilt; the two stay separate settings exactly as this section argues.
...
- ~~**Default `waypoint_types_shown`**~~ - **decided 2026-08-20** (#865): a curated subset, not all types on. Shelter, campsite, water and privy show on a fresh install; resupply, crossing, viewpoint and parking start hidden - reachable the same way any hidden category always was, through Settings or the legend. `lib/waypointVisibility.ts`'s `DEFAULT_SHOWN_TYPES` is the constant this settled to, and the Settings control renamed itself to "Default Waypoints shown" to say what it now sets.
...
- **Whether auto-rotate is worth its build cost** - the native `WKUIDelegate` work for the Capacitor iOS shell is real engineering effort for a feature that isn't safety-critical, unlike the now-removed wrong-way alert (HIKER_SAFETY.md §5). Worth weighing deliberately rather than assuming it's cheap because the ask itself is a short bullet point.
