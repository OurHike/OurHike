...
// WIREFRAMES.md §10, features/MORE_TAB.md. Five groups, one canonical
// UserPreferences model, each its own component since the More tab split
// into four tabs (#796) - More.test.tsx covers which tab shows which group
// and how AboutBuild/ReportBug/DownloadsLink are wired into the About tab.
// What is tested here is each group in isolation.
//
...
/** MapSettings with the props overridden, for the tests that care about one
 *  of them - the file otherwise spreads PROPS directly. */
function renderMap(overrides: Partial<MapSettingsProps> = {}) {
  return render(<MapSettings {...PROPS} {...overrides} />)
}
...
describe('YouSettings', () => {
  ...
  it('shows the account and offers sign-out when signed in', async () => {
    const user = userEvent.setup()
    render(<YouSettings {...PROPS} account={{ email: 'pat@example.org' }} />)
...
describe('MapSettings', () => {
  ...
  it('writes the red-light toggle once night_hike is chosen', async () => {
    const user = userEvent.setup()
    renderMap({ preferences: { ...DEFAULT_PREFERENCES, map_style: 'night_hike' } })
  ...
  it('tells the hiker when Data Saver has overridden their background choice', () => {
    ...
    renderMap({ preferences: live, dataSaver: true, archiveDownloaded: true })
  ...
  it('stays quiet when Data Saver merely agrees with what was already picked', () => {
    ...
    renderMap({ preferences: offline, dataSaver: true, archiveDownloaded: true })
  ...
  describe('waypoints shown', () => {
    // The full category list (#530). WIREFRAMES.md §2 has always put it here
    // rather than in the legend, and the reason is one of the three
    // consequences that issue lists: the legend's rows are per-viewport by
    // design, so a category with no points in view has no row there and
...
describe('DataSettings', () => {
  ...
  it('says the data has never synced rather than leaving it blank', () => {
    render(<DataSettings {...PROPS} lastSyncedAt={null} />)
