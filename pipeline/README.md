...
Companion to [../TECHNICAL_ARCHITECTURE.md](../TECHNICAL_ARCHITECTURE.md). This is the Python + DuckDB (spatial) pipeline that turns upstream data into the packages the client app downloads. Five organizations publish the trail lines it ships — the Appalachian Trail Conservancy, NYNJTC, NYS OPRHP, NYS DEC and Mohonk Preserve — alongside USGS, OpenStreetMap and opentrail.org. `sources.json` is the registry that decides which; this line named only ATC, USGS and opentrail.org until 2026-08-27.
...
`sources.json` is the registry of every upstream layer we pull from — 33 entries across nine organizations, of which `discover_sources.py` generates the ATC ones rather than anybody hand-writing them. (It read "every upstream ATC layer" until 2026-08-27, which the same file already contradicted eighteen lines below.) The ATC doesn't publish a plain list of data downloads - their public map is an ArcGIS Experience Builder app - so this script walks the same chain a browser resolves at load time to find the real FeatureServer URLs underneath it:
...
A fourth shelf is designed and unbuilt, and it is deliberately **not in this repository and not in the public bucket**: a private R2 store holding `data/raw/`'s contents between runs, so a fresh CI runner starts from yesterday's bytes instead of re-fetching them ([INCREMENTAL.md](INCREMENTAL.md), designed 2026-09-09). Raw layers carry rows this project has decided never to publish, so that store is a second bucket with no public domain rather than a prefix on the one hikers download from. Nothing writes it yet; until it exists, `data/raw/` survives a run only through the Actions cache `publish-vector-data.yml` carries. A fifth, designed 2026-09-24 and also unbuilt, is a second private bucket for what the expensive steps *compute*, each output stored under a hash of its inputs so an unchanged step is not re-run (INCREMENTAL.md, "Three tiers, and a step cache between them").
...
`fetch_trail_water.py` writes `data/raw/trail_water.json` — gitignored, cached between CI runs, and published to R2 like every other fetched layer, because derived coordinates are data and [data does not go in commits](../CONTRIBUTING.md#data-does-not-go-in-commits): **which shelters and campsites have water they can actually walk to**, from **both** hydrographies — USGS's NHD and OpenStreetMap — merged rather than picked between, because they know different things: USGS classifies flow (perennial / intermittent / ephemeral), OSM more often carries the local name and is edited by people who walk there. A stream point deduped across the two keeps whichever half each supplied, records both in `sources`, and attributes the flow claim to whoever made it (`flow_source`) — [features/POI_DEDUPLICATION.md](../features/POI_DEDUPLICATION.md)'s combine-don't-drop rule. USGS arrives as bulk staged GeoPackages, one subregion at a time, downloaded read and deleted: its query service 504s under corridor-scale load, and a derivation nobody can re-run is not one anybody can check. OSM costs no network at all — it is the same Geofabrik extracts the basemap build already downloads.

**It derived stream crossings too, until [#1674 — Stream crossings clutter the map: remove the `crossing` waypoint type, its legend row, and the pipeline step that derives it](https://github.com/OurHike/OurHike/issues/1674).** Every exact intersection of the walking routes (ATC's centerline and, from #1016, the other organizations' trails) with either hydrography's stream lines, published as the `crossing` poi_type — 1,125 on the corridor when first measured, 571 seen by both databases, and 5,318 on production release 2026-09-04 once the network trails joined. The maintainer asked for them off the map and the step removed, so the intersection, the dedupe and the routes table are gone, `crossing` is in `lib/poi_schema.WITHDRAWN_POI_TYPES`, and the identity ledger retired its rows (`reconcile_poi_identity.py --withdraw`). Those figures are kept as what was measured (the #710 lineage finding that split them lives on `fetch_trail_water.NHD_LINEAGE_TAGS`); **39 of 512 shelters and campsites** getting water they can walk to is the figure that still describes the output.
...
Two thirds of the guidebook's stream rows have one of our crossings within a fifth of a mile, from hydrography that has never seen the guidebook — which is what a crossing claims. Springs are the structural gap and cannot be otherwise: a spring does not cross the trail. And 60% of our crossings are not guidebook water, which is why they published as `crossing` and never as water pins.

**What #1674 costs against this table, stated because it is a water question.** The crossings are withdrawn, so the middle column describes data the app no longer publishes. They were never water pins and never fed a water distance, so nothing a hiker reads as *water* moved — but the "any OurHike water" column counted them: of the guidebook's 460 stream rows, 320 had some OurHike water within 0.2 mi and 302 had a crossing, and how many of those had *only* a crossing is not in the recorded results. Without them the figure is lower by an amount **nobody has re-measured**: re-running `spike_guide_water_check.py` against a post-#1674 `trail_water.json` would give it, and it needs the maintainer's personal copy of the guide.
...
## Fetching another organization's hikes, and forming the routes they do not publish ([#1427](https://github.com/OurHike/OurHike/issues/1427))

`fetch_hikefinder.py` reads NYNJTC's full hike list out of the [Hike Finder export](https://www.heardimmunity.org/hikefinder/) into `data/raw/hikefinder.json` — the entry of `kind: "published_hikes"` — with the published GPX track for every hike that has one into `data/raw/hikefinder_gpx/`. **This replaced the scrape of nynjtc.org on 2026-09-15**, on the maintainer's instruction ("Can you load all the hikes from here, and stop scraping the website"): `fetch_nynjtc_hikes.py` read their WordPress API and reached 20 hikes of 59, because the other 39 sit behind NYNJTC's own password. Measured 2026-09-15 against the live export: **385 hikes listed, 385 parsed, none refused**, every one carrying a stated length, a difficulty, a route type, a park, a region and a coordinate. `lib/hikefinder.py` is the reading, and it keeps everything the page says — including the **Features tags** (13 distinct across the corpus: Views 313, Historic feature 148, Waterfall 58, Cliffs 56, …), carried as published because they are the facets [#1284](https://github.com/OurHike/OurHike/issues/1284)'s finder filters on. The permission is unchanged and is the maintainer's relay of NYNJTC's, recorded in `sources.json`'s `nynjtc_hikes_licence`; only the road changed.

```
.venv/Scripts/python fetch_hikefinder.py              # all 385 hikes and the 113 published tracks
.venv/Scripts/python route_hikefinder.py              # form or read a route for each, graded; write the sheet
.venv/Scripts/python export_suggested_hikes.py        # what passes, as suggested_hikes.json
```

**A route arrives two ways, and the difference is carried all the way to the card.** 113 of the 385 publish a GPX track the writer drew — the first route in this repository that NYNJTC's hikes did not need OurHike to construct — and it ships exactly as drawn, never snapped to this build's lines. The other 272 publish a parking pin and a turn-by-turn description and no line at all, so [`lib/hike_route_builder.py`](lib/hike_route_builder.py) forms one: it snaps the parking, reads the trails the description names **in the order it names them**, resolves each to the nearest point on that trail *to where the walk has reached* — which is the junction the sentence means — and routes through them with [`lib/trail_graph_route.py`](lib/trail_graph_route.py), the Python twin of the phone's router. Every formed route is then graded against what the publisher independently said: its own stated mileage, its own route type. `route_hikefinder.py` renders the sheet a person reads to disagree with any of it.

**What that produced on 2026-09-15**, against the 2026-09-14 production graph (631,915 edges):

| | strong | fair | rejected |
|---|---|---|---|
| published (a GPX line the publisher drew) | 110 | 3 | 0 |
| generated (formed from the description) | 43 | 111 | 118 |

**The grade is calibrated, not chosen.** 113 of the export's hikes publish *both* a turn-by-turn description *and* a GPX line the publisher drew along the trails they were describing, so a route formed from the prose alone can be scored against the publisher's own statement of where the walk goes. (Those files are drawn rather than recorded — 110 of 113 come from gpx.studio and only 2 carry a `<time>` element — which makes them a statement of intent rather than a survey, and #1451 has the measurement.) That ground truth sets the bands in `lib/hike_route_builder.py`'s `_grade`: `strong` selects routes matching the publisher's drawn line **92%** of the time, measured, rather than routes clearing a threshold somebody liked the look of.

It settled what does *not* work, which is worth as much. Grading on how closely the measured length matches the publisher's stated one — the whole of the first version — carries **no signal** once the search is allowed to fit that length: across four bands of disagreement the share of routes matching the true track ran 54%, 77%, 54%, 58%. What carries signal is how much of the description the walk covers (71% above 0.9, 20% below 0.6) and whether a closed walk doubles back (89% in the 0.05–0.30 band).

The rejections are honest rather than tunable: 44 walks cannot be fitted to the stated length within 40%; 31 trailheads sit more than 500 m from any line in the layers registered here ([#1293](https://github.com/OurHike/OurHike/issues/1293)); 17 find no path and 17 name no trail in reach. **The remaining 44 are mostly a data gap rather than a search failure** — of the trails these descriptions name, 17% have nothing resembling them within reach, because no registered layer draws them.

**201 of the 385 reach a phone**, and the gap is a client limit rather than a data one. `SuggestedHike.segments` is "the ends, never the route", so a published line ships only when re-routing between its snapped samples reproduces it: 64 of the 113 tracks snap cleanly and 47 pass, while 49 leave the trails drawn here entirely. The 66 that cannot ship keep their full geometry in `hikefinder_routes.json` and `data/raw/hikefinder_gpx/`; [#1428](https://github.com/OurHike/OurHike/issues/1428) is the `track` field and the artifact split that would let them through.
...
So **six** of the eight external sources now carry `reaches_hikers: true` and ship: `oprhp_trails`, `oprhp_trail_closures`, `nynjtc_long_path`, `nynjtc_highlands_trail`, `mohonk_trails` and `dec_hiking_trails` — the last three added since this paragraph was written, by [#992](https://github.com/OurHike/OurHike/issues/992) and [#1019](https://github.com/OurHike/OurHike/issues/1019), each on the maintainer's authorisation because none of those three stewards states terms. The closure layer left the held-back group under [#964](https://github.com/OurHike/OurHike/issues/964), which derives it onto the trail lines. The two that stayed false longest — OPRHP's facilities and park polygons — were false for a different reason: registered and **nothing exported them yet**, which is `reaches_hikers`'s other meaning and worth keeping distinct from a licence hold. Both have since flipped the day an exporter first read them: the facilities under [#1097](https://github.com/OurHike/OurHike/issues/1097) (`export_nearby_poi.py`), the park polygons under [#1371](https://github.com/OurHike/OurHike/issues/1371) (`export_places.py`, below). OPRHP's required attribution rides in `client/src/map/credits.ts` and appears in the map corner whenever their lines are drawn; `export_sources.py` names both stewards on the sources screen. The [#771](https://github.com/OurHike/OurHike/issues/771) spike was this data's first consumer and the export is its second.

## Exporting the trails other organizations maintain (#950)

`export_nearby_trails.py` turns what `fetch_external_layers.py` downloaded into the lines the map draws behind the chosen trail — [features/NEARBY_TRAILS.md](../features/NEARBY_TRAILS.md)'s map, given data to draw. It is a second export rather than a branch inside `export_trails.py` because the two differ in all three things that matter: where the raw file is (`data/raw/external/` against `data/raw/`), what it is clipped to (nothing at all, since [#1019](https://github.com/OurHike/OurHike/issues/1019), against the 30-mile A.T. corridor), and whether it may be published at all.

Where two of those trails run on one treadway — the Ramapo-Dunderberg on the A.T., the Long Path on the Escarpment Trail — the same export writes `concurrent_trails.geojson` beside the lines ([#1384](https://github.com/OurHike/OurHike/issues/1384)): for every stretch where one trail's lines lie within 10 m of another's for 50 m or more, a pair of features on one chord, each trail's own properties plus `concurrent_with`, `concurrent_source` and a `concurrent_side` of +1 or −1, so the map can offset each blaze to its own side and weigh the stretch at the heavier of the two tiers. [`lib/concurrency.py`](lib/concurrency.py) is the pairing and carries both measurements. The A.T.'s centerline is in the pool, read from the raw fetch because this export runs before `export_trails.py` does; a checkout without it pairs the network alone and says so. The pairs ride **into the vector tiles only** — `nearby_trails.geojson` is unchanged, because `build_trail_graph.py`, `fetch_trail_water.py` and nine other scripts read it as the network's topology and a stretch published twice more would be a duplicate edge to each of them. Only stretches whose two halves carry two different real blazes are kept: measured 2026-09-10, most of what the pairing found first was one trail spelled twice by two organizations, in the same paint or none.
...
| Status | maintainer, 2026-08-18 | 43 OPRHP segments with a blank, `Proposed` or `Unknown` status (`Closed` **ships**, drawn as barrier tape). No other source publishes a status column |
...
## Exporting the places a hiker can name (#1371)

`export_places.py` writes `places.json`: the parks, towns, trailheads, parking areas and long trails a hiker can name on first run **before anything is downloaded** — the "where do you hike?" step the front-end rebuild adds, whose answer becomes the map's fallback centre for every fix GPS never gets. The same rows feed step 2's stop picker, the finder's location facet and the volunteer map's place search, so one artifact answers a question four screens ask.

```
.venv/Scripts/python export_places.py
```

Runs after `export_trails.py`, `export_nearby_trails.py`, `export_poi.py` and `export_nearby_poi.py`, and reads only what they wrote plus the raw layers they already read. Parks come from `oprhp_park_polygons` (858 parcel polygons grouped into their 258 park units by `MasterAreaID`, measured live 2026-09-10 — the first exporter to read the layer, which is what flipped its `reaches_hikers`) as a name, OPRHP's own category, a centroid and a bounding box over the whole unit — never the polygon. The miles are measured over the lines whose stewards carry `reaches_hikers: true` — the same gate `publish.py` applies to `nearby_trails.geojson`, applied one file earlier so a held-back steward's lines count for nothing — and the A.T. Towns are the published A.T. Communities with the raw layer's `STATE`; trailheads and parking are the published waypoints of those types, named rows only, carrying the waypoint's own id so a pick is also a waypoint the app holds, plus the park each one sits inside. The long trails are every named line totalling at least `NAMED_TRAIL_THRESHOLD_MILES` (`export_nearby_trails.py`'s 50, imported rather than copied) and the A.T.

**"Miles of trail held" is measured over the lines this run published** — a park's is the length of published line inside its boundary; a point's is the length within `PLACE_TRAIL_RADIUS_MILES` of it, and the artifact carries that radius (`trailRadiusMiles`) so a screen prints what was measured. The radius is **@unvalidated** at 5 mi; the module docstring says what would settle it. When no line was published at all, `trailMiles` is omitted on every row and the document says `trailMilesMeasured: false` — absent means unknown, never zero. Trailheads and parking with nothing published within the radius are dropped and counted in the manifest (a USFS trailhead in Arizona is not a place this app can put a trail under — [#1231](https://github.com/OurHike/OurHike/issues/1231)'s concern, one artifact over); towns and parks with nothing are kept, because "no trail data held" is the true sentence a hiker who lives there should be able to find. `publish.py` collects it as an optional artifact; the client reader lands with the rebuild.
...
It's `check_freshness.py`'s output-side sibling: `check_freshness.py` (run before fetching) asks whether anything *upstream* has changed; this asks whether *this run's own output* can be trusted, after everything has already run. Seven checks, in priority order - see the module's own docstring for the full reasoning behind each, especially the corridor one:
...
6. **OSM water reachability** ([#916](https://github.com/OurHike/OurHike/issues/916)) — check 1's logic applied to a claim rather than a count. The five above would all pass a water layer whose every point sat thirty miles from the trail: the counts are right, the hashes match, the corridor agrees, the receipts are there. This re-reads `data/processed/poi/water.geojson` — the file that was actually written — and re-measures every `osm_water` point against the raw layers [#749](https://github.com/OurHike/OurHike/issues/749)'s gate itself read, requiring each one inside the same **100 ft** union — which since [#1016](https://github.com/OurHike/OurHike/issues/1016) includes the published network lines, so this checker cannot fail a point the gate correctly passed on somebody else's trail. The distance half of that gate only, never the grade half, which would need a network round trip per point; `check_water_reach.py` holds both the check and the measurement that prompted it.
7. **JSON a phone can parse** — check 1 re-hashes every artifact and confirms the bytes are the ones the exporter wrote; nothing confirmed a phone could read them. Python's `json` module writes a non-finite float as the bare token `NaN` or `Infinity` and reads either back without a word, and `JSON.parse` in the WebView rejects the whole document on the first one ([#659](https://github.com/OurHike/OurHike/issues/659) found exactly that in the elevation profile). DuckDB 1.5.5 answers `0.0/0.0` with `nan` and `pyproj` hands back `inf` for a point it cannot place, so the value has real ways in; measured against production on 2026-09-17, none of 236 JSON artifacts carried one. This reads every `.json` and `.geojson` under `data/processed/` the strict way (`lib/strict_json.py`) and fails on the file by name — a document that does not parse at all, which is what a write cut short looks like, fails the same way.
...
## The 1-degree coverage cells (#1175)

The unit of offline coverage is the graticule cell — the maintainer's #552 call
(2026-08-25): a 1°×1° square, gathered into a named *piece* a hiker actually taps
(`features/OFFLINE_COVERAGE.md`). `cut_cells.py` cuts them from the corridor archives, by
tile rather than by polygon: a tile's bounds and a cell's bounds are both axis-aligned
rectangles in lon/lat, so membership is an exact overlap test rather than the five-point
projection the trail-derived cut needed. Everything through z9 becomes ONE shared
`*_context.pmtiles` per sheet instead of riding in every unit (#193's duplication,
answered — at 51 cells, per-cell context would be ~321 MB of the same bytes), and
`<family>_cells.json` publishes the coverage index: cell names, keys, and core bounds.
`verify_release.py` check 20 fails a release naming a cell it never published or naming
one that is not a whole square, and `latest.json` carries a measured `size_bytes` per
artifact (#505's third ask), so per-unit download prompts never advertise a hand-kept
number.

Cells are anchored to whole degrees, never to an archive's bounding box, and that is the
whole point: `lib/corridor_grid.graticule_cells` exists beside `compute_cells` rather than
replacing it, because two organizations' sheets over the same ground must produce the
*same* cells or the overlap that cells were chosen to eliminate comes straight back.

**Supersedes the 50-mile stretch cut (#556)**, which built the unit the decision replaced
and went on publishing it for three days — 88 archives, 942.9 MB, read by zero client
code. Those keys stay in the bucket (the manifest merge is additive-only, so a name can
