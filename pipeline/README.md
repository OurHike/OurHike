...
Where two of those trails run on one treadway — the Ramapo-Dunderberg on the A.T., the Long Path on the Escarpment Trail — the same export writes `concurrent_trails.geojson` beside the lines ([#1384](https://github.com/OurHike/OurHike/issues/1384)): for every stretch where one trail's lines lie within 10 m of another's for 50 m or more, a pair of features on one chord, each trail's own properties plus `concurrent_with` and a `concurrent_side` of +1 or −1, so the map can offset each blaze to its own side. [`lib/concurrency.py`](lib/concurrency.py) is the pairing and carries both measurements. The A.T.'s centerline is in the pool, read from the raw fetch because this export runs before `export_trails.py` does; a checkout without it pairs the network alone and says so. The pairs ride **into the vector tiles only** — `nearby_trails.geojson` is unchanged, because `build_trail_graph.py`, `fetch_trail_water.py` and nine other scripts read it as the network's topology and a stretch published twice more would be a duplicate edge to each of them. Only stretches whose two halves carry two different real blazes are kept: measured 2026-09-10, most of what the pairing found first was one trail spelled twice by two organizations, in the same paint or none.
