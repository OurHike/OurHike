...
**The dbt layer tests itself twice over (#100, pipeline/DBT.md):** `dbt build` runs the models' own generic and singular tests - column constraints, and the row-count reconciliation tests that hold each layer 1:1 with its inputs - while pytest covers the Python side: `test_load_raw.py` (the registry decides what loads, bookkeeping columns, replace-not-append) and `test_dbt_seed_sync.py`, which fails the moment `poi_type_mapping.csv` drifts from the `ICON_LEGEND`/`OPENTRAIL_ICON_MAP`/`DIRECT_SOURCES` constants it transcribes. CI's `dbt` job runs the SQL half against `make_dbt_fixtures.py`'s synthetic layers on Python 3.12 (dbt lags the pytest job's bleeding edge, pinned deliberately); the pytest half rides the ordinary pipeline suite.
...
**The flow layer is the second half of that boundary** (#1386): `client/e2e/` drives the built app in a real browser, by real taps, to prove a screen is *findable* and *exitable* rather than merely rendered - the rung `src/test/reachability.test.ts`'s own comment names and cannot reach ("imported is weaker than rendered, and rendered is weaker than findable"). [features/FLOW_TESTING.md](features/FLOW_TESTING.md) is how those are written and what is still to write; `src/test/flowCoverage.test.ts` is what stops the list rotting, by failing this suite when a screen module has no answer.
...
21. **A screen's load is counted in work, not in milliseconds — and the stopwatch is a separate thing that is not a gate.** #857 was reported as a feeling ("when I hit skip on the first 3 screens there is some serious lag… it feels like that button is broken") and was real: 5,479 ms of blocking work in 22 long tasks while the entry steps were up, the longest of them 2,374 ms, measured in Chromium at 390×844 on a 4× CPU throttle. None of that is measurable in the suite — jsdom has no compositor, no WebGL, no tile worker and no paint, `maplibre-gl` is mocked outright (§19), and a CI runner's milliseconds are not a phone's, so a timing assertion tuned on one machine is exactly the flaky test CLAUDE.md warns about. What *is* deterministic is how much work a launch asks for, because every millisecond in that measurement came from a named operation: rasterising the 46 pin images, parsing 12.2 MB of trail lines, indexing 219,293 vertices, deserialising 2,837 POIs. So `client/src/App.loadBudget.test.tsx` spells the budget in calls — which of those a screen may perform before it is on screen, and how many times — and fails loudly on a machine of any speed when the work comes back. The stopwatch lives in `client/scripts/measure-first-run.mjs`, which drives a real throttled Chromium against a real deployment (a pull request's preview URL, or production) and prints the tap latencies, the long-task timeline and a source-mapped profile. Run it when touching the launch path and paste what it prints into the pull request; it is deliberately not in `npm test` or in CI, because a number it produced on a shared runner would be a number nobody could act on. The same file holds a second budget since #1192 — *A returning hiker's launch freezes for ten seconds while every waypoint is placed on the trail, on the main thread* — for the launch every day after the first: once the index has landed, the launch thread runs the nearest-vertex search for **no** waypoint at all, because the placement happens where the index is built (`lib/trailIndexBuild.ts`, a worker on a phone). That budget was missing when it was needed: 16,949 placements in one memo cost 13,078 ms on the throttled profile and no test counted them. The stopwatch grew `--returning` for the same reason — it measured first run only, which #857 had fixed, and the launch the report was about had no instrument.
...
Built. pytest, 167 tests (measured 2026-08-06), reusing the pipeline's approach - synthetic fixtures, no network (every JWT is minted locally; the JWKS fetch is a monkeypatched seam, never called). The one deliberate departure from "mock the heavy thing": the suite runs against a **real local Postgres** - the same engine Supabase runs in production - never a stand-in dialect, because RLS statements, Alembic migrations up *and* down, and `pg_class` readbacks are exactly the things a stand-in would vouch for wrongly. `scripts/local-postgres.sh` makes that a one-command setup. The versions are close rather than identical, said precisely because this line used to overclaim (#658): CI pins `postgres:17` to track the Supabase project's 17.6, while the local script runs whatever major the machine has - the sandbox ships 16 - so a 16-vs-17 behaviour difference is caught by CI, not locally.
...
**The suite also holds claims about the *set* of workflows,** which is the other thing no checkout of one file can answer. `test_supabase_keepalive_workflow.py` is the current example: every scheduled workflow that reaches the Supabase project has to be a named, justified entrant on its `SCHEDULED_ROSTER`, which fails on an unnamed fourth road the same way it once failed on a second at all — the "exactly one" version of this test (#656) could not tell a real second keepalive from `schema-drift.yml` reading the same project's databases for an unrelated reason, so it was replaced with a census that asks for a name and a reason instead of a count. And it fails on the *keepalive's own* schedule quietly running less often than it looks, which is the one part of that job with no feedback loop of its own. It asserts the **longest gap the cron leaves**, not the string it is written as: `50 */20 * * *` reads like "every 20 hours" and actually fires 20 hours apart and then 4, so a test pinning the text would have said nothing about the thing that decides whether the job works.
...
The client is one codebase for all three platforms: a PWA, wrapped with Capacitor for the app-store builds (TECHNICAL_ARCHITECTURE.md). **The wrapper exists** - `client/ios/` and `client/android/` are in the tree, and `build-shells.yml`'s **iOS compile check** and **Android debug APK** jobs build both in CI (#1394 corrected the paragraph that said otherwise, which had been true when written and outlived #101). What has not changed is what a green unit run attests to, which is the part of this section worth keeping: "will the tests work on Android and iOS" has a precise answer today: the suite runs anywhere Node runs and will keep passing unchanged after wrapping, but it *attests* to web semantics only. jsdom is one idealized browser; nothing in a green run speaks for WKWebView on iOS or the Android System WebView. And those are where this app's riskiest behaviors genuinely differ:
...
The posture, now that Capacitor has landed: the unit suite stays platform-agnostic and does not fork. What gets added is a thin per-platform smoke layer, not a parallel suite - Playwright's WebKit and Chromium builds are the same rendering engines the two WebViews embed, which makes an engine-level smoke check cheap and runnable in CI - and [features/FLOW_TESTING.md](features/FLOW_TESTING.md) is that layer, with **#1392 - The flow suite runs one project: no desktop viewport, and WebKit has never been run at all** as the issue for turning the second engine on. The truly device-bound behaviors - storage eviction, permission prompts, background GPS under canopy - belong to Phase 3 acceptance runs on real devices and to field testing: a documented manual procedure, same category as the full USGS fetch.

**Half of that layer now exists and the platform half of it does not**, said in the same breath as the plan so the plan is not read as done. `client/e2e/` runs the real bundle in real Chromium - the engine the Android System WebView embeds - so the Android side of "engine-level" has a home. **WebKit has never been run here**: `/opt/pw-browsers` carries Chromium alone, and a project whose first execution is in CI is a project whose first result is a red pull request, so turning it on is a deliberate act by somebody who can run it locally first. [features/FLOW_TESTING.md](features/FLOW_TESTING.md)'s "Engine" axis carries that as an `@unvalidated` gap with what would settle it. Nothing about the flow layer moves the device-bound line above: **Appium, not Playwright, is the tool** if permission prompts and background GPS are ever automated, and this layer does not pretend to reach them.
...
- ~~**The one existing contract test is one-sided in CI.**~~ Fixed in the v1 review (#317). The rule it left behind is the one that matters, and it now has three files depending on it: **a suite's scope list includes every file its tests read.** The client suite reads `pipeline/reference/gain_vectors.json` and `site/index.html`; the backend and pipeline suites each read four modules under `client/src/lib/`. All three scope lists say so, and each says why - because the drift a cross-language guard exists to catch usually arrives in a pull request that touches only the *other* language, which is exactly the run a same-directory scope would skip.
  ...
  **The Python suites name those files individually rather than listing `client/src/lib/`**, and that is a deliberate second decision rather than fussiness. The directory is touched by most client work, so the broad prefix stood up a Postgres container and ran two Python suites on nearly every client pull request in order to check eight modules that change a few times a year. A tax that size on the commonest kind of change here is how a rule ends up quietly relaxed later.
  ...
  What makes the narrow list safe is that it is checked: `backend/tests/test_ci_scope.py` and `pipeline/tests/test_ci_scope.py` parse their own workflow and compare its scope list against the paths the contract tests declare (`CLIENT_FILES_READ`), so a contract test that starts reading a fifth client file fails there rather than silently ceasing to run in CI. This is the general shape worth copying: **narrow the scope list as far as you like, provided something fails when it goes stale.** The client suite now has its equivalent (#503), by a different mechanism: finding a Vitest suite's out-of-tree reads by parsing test source would be a regex over dissimilar call shapes - the kind of parser that fails open - so instead every out-of-tree read goes through `client/src/test/repoFile.ts`, which only serves paths declared in its `OUT_OF_TREE_READS` list, and `ciScope.test.ts` asserts that list against the workflow's scope list. The declaration is a precondition of doing the read at all, rather than something a scanner infers afterwards.
- ~~**The guard has no guard.**~~ Closed by #319. `client/src/test/checkBuildOutput.test.ts` runs the real `check-build-output.mjs` — the same entry point `npm run build` runs — against tiny synthetic `dist/` trees, and proves it goes red on each defect class it guards: a referenced asset never emitted, the worker missing or unwired, a missing service worker, an asset outside the precache, a wrong glyph or font count, cross-origin CSS, and MapLibre's stylesheet rules absent from the bundle. Writing the tests found and fixed one real hole: the reference scan included `sw.js`, whose precache manifest names every precached asset, so anything precached counted as "wired up" — which silently disarmed the exact blank-map check the script was written for.
- **Storage's CI layer is no longer wholly simulated** (#318): `client/src/lib/archiveDownload.realIdb.test.ts` runs the download engine through the real `idb-keyval` against fake-indexeddb's real object stores - a download read back byte for byte, a resume from really-persisted segments, and a quota-exhaustion mid-write that leaves the finished archive intact (the DOMException is injected at the store boundary, since fake-indexeddb enforces no quota; jsdom's own Blob is opaque to structuredClone, so the file substitutes node's, which is why the rest of the suite still mocks the store). What remains between that and a full phone is device-bound, and the probe covers it by hand: `client/scripts/storage-probe/`, which drives the real download into a real Chromium at a real size, by hand. It is what found #544 - the Fine tier transferring all 1.18 GB before failing to store it, because the Blob being accumulated is not charged against the origin's quota, so nothing refuses until the final `set()`. No jsdom test can see that, and this one saw it in a minute. It is deliberately not a gate: it moves gigabytes and its numbers depend on the machine's free disk. **The gap that remains is that nothing runs it on a schedule**, so a regression in the download path is caught by whoever thinks to look.
...
3. **A real-browser layer lands between jsdom and the trail.** Not a broad E2E suite - a smoke: the built app boots in real Chromium and WebKit, constructs a real map with its real worker, completes one small archive download against real IndexedDB, and comes back up offline. Its job is to catch the classes of bug the mocks structurally cannot - item 19's blank map is the acceptance test for it - and it doubles as the platform smoke layer Phase 3 needs anyway.
   ...
   **Started, in Chromium, at #1386** (`client/e2e/`, run by `npm run test:e2e`): the built app boots in a real browser and is driven by real taps through first run, Today, step 1 of the planning spine and the bail sheet, with [features/FLOW_TESTING.md](features/FLOW_TESTING.md) holding how a flow test is formed, the six axes each flow is asserted across, and the battery still to write. What has *not* landed from the sentence above is the part worth not losing: **WebKit has never been run** - no environment this was written in has it installed - and none of the three smoke items, a real map with its real worker, an archive download against real IndexedDB, coming back up offline, is driven yet. So `check-build-output.mjs` is still the only thing standing between item 19's blank map and a release, which is why it still gets tests of its own.

   The half of this that is structural rather than a to-do list: `client/src/test/flowCoverage.test.ts` fails the ordinary client suite when a screen module has no flow-coverage answer, so the battery's remaining rows are a list the repository keeps rather than one somebody remembers.
