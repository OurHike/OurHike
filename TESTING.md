...
- **The backend↔client seam is checked at its edges, not down the middle.** The enums either side of it are now compared (above), and `scripts/check_openapi_compat.py` holds this build against every supported release's document. What still has no second layer is the *shape of a response*: `client/src/lib/api.ts` is tested against hand-written mock bodies, `ReportSummary` and `ClosureSummary` are re-declared as TypeScript with nothing keeping them in step with `ReportOut`/`ClosureOut`, and no PR runs both halves of one request. The backend already emits an OpenAPI document - that check belongs against the document, not against a second hand-kept copy.
- ~~**The one existing contract test is one-sided in CI.**~~ Fixed in the v1 review. The rule it left behind is the one that matters, and it now has three files depending on it: **a suite's scope list includes every file its tests read.** The client suite reads `pipeline/reference/gain_vectors.json` and `site/index.html`; the backend and pipeline suites read `client/src/lib/`. All three scope lists say so, and each says why - because the drift a cross-language guard exists to catch usually arrives in a pull request that touches only the *other* language, which is exactly the run a same-directory scope would skip.
