...
**A route arrives two ways, and the difference is carried all the way to the card.** 113 of the 385 publish a GPX track the writer **drew** — the first route in this repository that NYNJTC's hikes did not need OurHike to construct — and it ships as published, never snapped to this build's lines. ([#1451](https://github.com/OurHike/OurHike/issues/1451): 110 of the 113 are gpx.studio files and 2 carry any timestamp at all, so these are drawn routes rather than recorded walks.) The other 272 publish a parking pin and a turn-by-turn description and no line at all, so [`lib/hike_route_builder.py`](lib/hike_route_builder.py) forms one: it snaps the parking, reads the trails the description names **in the order it names them**, resolves each to the nearest point on that trail *to where the walk has reached* — which is the junction the sentence means — and routes through them with [`lib/trail_graph_route.py`](lib/trail_graph_route.py), the Python twin of the phone's router. Every formed route is then graded against what the publisher independently said: its own stated mileage, its own route type. `route_hikefinder.py` renders the sheet a person reads to disagree with any of it.

**What that produced on 2026-09-15**, against the 2026-09-14 production graph (631,915 edges):

| | strong | fair | rejected |
|---|---|---|---|
| published (the publisher's own drawn GPX track) | 110 | 3 | 0 |
| generated (formed from the description) | 43 | 111 | 118 |

**The grade is calibrated, not chosen.** 113 of the export's hikes publish *both* a turn-by-turn description *and* a GPX track the publisher drew, so a route formed from the prose alone can be scored against the line the publisher themselves put down. That ground truth sets the bands in `lib/hike_route_builder.py`'s `_grade`: `strong` selects routes matching the publisher's own drawn line **92%** of the time, measured, rather than routes clearing a threshold somebody liked the look of. The calibration is unaffected by [#1451](https://github.com/OurHike/OurHike/issues/1451) — the question it asks is "did we reconstruct the walk this description describes?", and a drawn line answers that at least as well as a recording would. One nuance in our favour, recorded rather than claimed: gpx.studio routes along OSM while this build's graph is OPRHP, NYNJTC and DEC, so some of the 8% miss is two organizations tracing one trail differently rather than a routing error. Nobody has separated the two.
