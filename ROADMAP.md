...
This line used to include **the wrong-way cue**, and it was wrong to. Every component in the list existed, which is presumably how it came to be written ([#232](https://github.com/jaimito-asuntos-gringuenos/OurHike/issues/232)) — but a hiker on trail saw no closure, no warning pin and no wrong-way cue, because nothing mounted any of them and nothing fetched the data behind two of the three. The closures and the warning pins are real now. The wrong-way cue is still built and unmounted, deliberately: its thresholds are placeholders ([#93](https://github.com/jaimito-asuntos-gringuenos/OurHike/issues/93)), and it is the only notification this app sends, so false alarms would spend the trust budget that single alert was designed around. Tapping a closure or a warning to read the detail sheet is also still missing — both sheets render fields no backend can supply ([#245](https://github.com/jaimito-asuntos-gringuenos/OurHike/issues/245), [#292](https://github.com/jaimito-asuntos-gringuenos/OurHike/issues/292)).
