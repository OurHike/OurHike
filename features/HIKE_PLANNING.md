...
- **≈ walking time, where the ground under it has been measured.** `lib/dayHikeTime.ts` prices a day hike through the same `legFigures` the trip planner prices its own days with, so a stretch of centerline walked as a day hike and walked as a trip day come out at the same number. It is **all or nothing**: a walk with one step this phone cannot price answers null, and the builder bar and the card print no time rather than a low one. Today the priceable ground is `source === 'centerline'` — `build_trail_graph.py`'s own name for the A.T., and the only thing `export_elevation.py` measured — so a hike onto a blue blaze prices at nothing. Two reasons it refuses rather than approximates: pricing an unknown edge at zero ascent is a flat-ground claim about real ground, and pricing only the known edges understates the same way with a number attached. Both fail *short*, which is the direction that gets somebody caught by the dark. As elevation for the other layers is published this is the one function that grows a branch; every caller already handles the null.

**What the storyboard drew that deliberately did not ship, and what each waits on:** starter hikes ("laid out by the clubs that maintain them" — no club-laid route dataset exists anywhere in this repository; the storyboard itself calls the work editorial, related to #981's parking lots); gain/loss on network day hikes (the same missing profiles the time above works around, and a gain/loss pair wants a place on the card that the figures line has not got); ≈ time on the day-hike **list** and the trailhead door (both read the stored cache, and pricing needs the routing graph a list must not load); the turn list (#934's first-leg naming question, above); recording a finished walk (#982); freehand drawing (#983).
