...
import { dayLongDateLabel, longDayLabel } from '../lib/planDisplay'
...
import { WorkdayRow } from '../chrome/WorkdayRow'
import { WorkdayCalendar } from '../chrome/WorkdayCalendar'
...
export function Today({
  ...
  /**
   * THE CREWS SECTION'S OWN STATE (#1440, D18-D22).
   *
   ...
   * is not a setting. It is reset by leaving the screen, which is what
   * somebody coming back tomorrow morning wants.
   */
  ...
  /**
   * THE ONE STALENESS GATE THE WHOLE CREWS SECTION READS (#1440, D19).
   *
   * Two absences, and neither is ever hedged row by row - both replace
   * everything, in every view, because "a hedged invitation still reads as an
   * invitation" and sending somebody to a trailhead for a workday cancelled
   * on Thursday is this feature's own failure mode.
   *
   *   - `unchecked` - no artifact has been read. NOT the same claim as an
   *     empty one, and rendered differently: "could not check" must never
   *     read as "no club has asked" (#249's rule).
   *   - `stale`  - older than OPPORTUNITIES_STALE_MS.
   *
   * In the calendar that means an empty grid, and on the map a pinless one.
   */
  const crewsUnchecked = opportunities === null
  const crewsStale =
    !crewsUnchecked &&
    (opportunitiesAsOf === null || !opportunitiesUsable(opportunitiesAsOf, now))
  /** Everything downstream reads this one list - the out-today block, the
   *  filtered list, the calendar and both hiking headings. Four readings of
   *  one list, which is what stops them drifting. */
  ...
  const named: Record<string, ReactNode> = {
    ...
    volunteer: mode === 'volunteer' ? crewsScreen : volunteer,
    crews: walkingCrews,
    soFar,
