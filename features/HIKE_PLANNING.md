...
**What the storyboard drew that deliberately did not ship, and what each waits on:** starter hikes ("laid out by the clubs that maintain them" — no club-laid route dataset exists anywhere in this repository; the storyboard itself calls the work editorial, related to #981's parking lots); ≈ walking time and gain/loss on network day hikes (no elevation exists for network trails — the builder bar's own refusal); the turn list (#934's first-leg naming question, above); recording a finished walk (#982); freehand drawing (#983).
