...
`Dockerfile` + two Fly configs target [Fly.io](https://fly.io) - picked over Render specifically to avoid Render's free-tier sleep-on-idle behavior (a cold start on the first request after idle is a worse experience for something safety-adjacent than a small ongoing hosting cost). `fly.toml` deliberately keeps `min_machines_running = 1` for the same reason - see the comment in that file if that tradeoff ever needs revisiting.

**Two apps, one per environment** (`../RELEASING.md` §3): `fly.toml` is production, `fly.ua.toml` is UA. They differ in exactly three keys - the app name, and the two settings that let UA scale to zero between testers where production keeps a machine warm. Everything else has to track, so `tests/test_fly_config.py` compares them and names any key that has drifted; a second config file is otherwise the standard way for one of them to quietly stop describing the same service.

**Deploying is `.github/workflows/deploy-backend.yml`, not a laptop.** A push to `main` deploys UA, an annotated `v*` tag deploys production behind the `production` environment's reviewer, and a dispatch against a previous tag is the rollback. What still has to be done by hand is the account work the workflow deliberately does not do - creating the two apps, setting the runtime secrets on them, and minting a `fly tokens create deploy` token per app into `UA_FLY_API_TOKEN` and `PRODUCTION_FLY_API_TOKEN` (two, because a deploy token is scoped to one app; the alternative is an org token that can delete both). `../LAUNCH_CHECKLIST.md` 6 is that setup, step by step; it is not repeated here.

Two things worth knowing that are properties of this directory rather than of the checklist:

- **Migrations are not part of a deploy** and stay that way - the Dockerfile says why, and `.github/workflows/migrate.yml` is where they run instead. Applying an additive revision before the deploy that uses it is the usual order, but `../RELEASING.md` §8c's expand-and-contract rule is why nothing decides that automatically.
- **Report photos (#234) need five secrets on the app**, and they are **not** the `R2_*` four the pipeline publishes with: `R2_PHOTO_ENDPOINT_URL`, `R2_PHOTO_BUCKET`, `R2_PHOTO_ACCESS_KEY_ID`, `R2_PHOTO_SECRET_ACCESS_KEY`, `R2_PHOTO_WRITE_ENABLED=true`. `R2_PHOTO_BUCKET` must name a **private** bucket - see `../LAUNCH_CHECKLIST.md` 1.7 and the comment in `app/config.py` for what pointing it at the published one would publish. Leaving all five unset is a valid deployment: it simply cannot take or serve photos, and both endpoints answer 503 rather than failing.

**None of it has run against a real Fly.io account or Docker daemon** - this sandbox has no Fly.io account, and while the Docker CLI is on its PATH, no daemon is running behind it (the same reason `docker-compose.yml` is documented but unexercised here, and why `scripts/local-postgres.sh` uses the container's own Postgres install instead). The Dockerfile follows a standard, well-established FastAPI/uvicorn pattern, both configs match Fly's own documented format, and the workflow's YAML and shell are checked by `.github/tests/` - but "should work" isn't the same claim as "confirmed working". Budget for the first real deploy to surface something this couldn't catch locally.
