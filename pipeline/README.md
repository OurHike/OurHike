...
Runs after `export_trails.py`, `export_nearby_trails.py`, `export_poi.py` and `export_nearby_poi.py`, and reads only what they wrote plus the raw layers they already read. Parks come from `oprhp_park_polygons` (858 boundaries; the first exporter to read it, which is what flipped its `reaches_hikers`) as a name, OPRHP's own category, a centroid and a bounding box — never the polygon. Towns are the published A.T. Communities with the raw layer's `STATE`; trailheads and parking are the published waypoints of those types, named rows only, carrying the waypoint's own id so a pick is also a waypoint the app holds, plus the park each one sits inside. The long trails are every named line totalling at least `NAMED_TRAIL_THRESHOLD_MILES` (`export_nearby_trails.py`'s 50, imported rather than copied) and the A.T.
