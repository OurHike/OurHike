...
- **A screen a hiker can reach comes with a flow test, in the same pull request.** [features/FLOW_TESTING.md](features/FLOW_TESTING.md) is how one is formed and the five axes it is asserted across — mode (all three, volunteer included), device, engine, data freshness, and a seeded hike rather than an empty install. Checked rather than remembered: `client/src/test/flowCoverage.test.ts` fails the client suite when a screen module has no entry in the coverage ledger, and `planned` is a real entry. A battery written after the feature is written against the code rather than against the intent, by someone who has already forgotten which states were meant to exist.
