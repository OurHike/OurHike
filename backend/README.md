...
2. Set the real runtime secrets there - never committed, never baked into the image: `DATABASE_URL` (the **transaction** pooler, port 6543, which is a different value from the session pooler the migrate job holds on 5432), `SUPABASE_URL`, `SUPABASE_ANON_KEY`.
   - **`SUPABASE_JWT_SECRET` is deliberately not in that list**, and used to be. A hosted Supabase project signs with ES256 and publishes the public half as a JWKS, so there is no shared secret to set; `app/config.py` defaults it empty for exactly that reason and `../LAUNCH_CHECKLIST.md` 4.4 says so. Set it only against a self-hosted Supabase, which this deployment is not.
...
**The image builds and runs, measured 2026-09-09 (#600)** - built unmodified and run against a real Postgres, `PORT` honoured at a non-default value and at its 8000 fallback, `/health` 200, no tables created by booting, and RLS on every table `alembic upgrade head` creates (18 of 18, re-measured 2026-09-09). `../LAUNCH_CHECKLIST.md` 6 carries the full result and, more usefully, what it still does not cover: a direct connection rather than Supabase's transaction pooler (#95), and no real Supabase project.

**It runs as `ourhike`, uid 999, not root** (#1358), and the matrix above was re-run against the hardened image rather than assumed to survive it - the migration included, since that is where a permissions mistake would surface. Fixing that also caught `.dockerignore` shipping 81 `.pyc` files into the image: its `__pycache__/` line matched only the top level, because Docker matches a pattern against the whole path rather than against every segment. The file now says so, at the one place somebody would otherwise repeat it.

**The daemon claim this paragraph used to make was wrong, and it was load-bearing.** It said the Docker CLI sits on this sandbox's PATH "with no daemon behind it", and gave that as the reason `docker-compose.yml` is unexercised and `scripts/local-postgres.sh` uses the container's own Postgres. A daemon starts here (`dockerd`, measured 2026-09-09 in a web session; 29.3.1, overlayfs, buildkit) - which is how the run above happened at all. So `docker-compose.yml` is now exercis**able**, though still unexercised, and the `local-postgres.sh` choice stands on its own merits rather than on this. One caveat for whoever tries next: the session's egress proxy re-terminates TLS, so `pip` inside a build cannot verify PyPI until the build is given the proxy CA - `/root/.ccr/README.md` documents the ways, and injecting it via a `--build-context` base override keeps `Dockerfile` itself untouched.
