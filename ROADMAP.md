...
Six features have full design drafts, written 2026-07-28 and now organized in [features/](features/) so nothing gets lost. Each is already linked in context from FEATURES.md and from the phase items above — this section is just the one place that lists all six together:

- [features/AUTHENTICATION.md](features/AUTHENTICATION.md) — Google/Apple/email sign-in, verification, optional MFA. Post-MVP, but build first — Segments, Volunteering, and Report a Problem all depend on it.
