...
**1.6 Publish — now a CI workflow, not a local script.** Dispatch **"Publish vector data"** (Actions tab → workflow_dispatch) with `publish` ticked. Leaving it unticked does a dry run: builds and quality-checks everything without uploading, useful for checking upstream still parses. `include_elevation` adds ~25 min and builds **three** artifacts the app **does** read: `elevation_profile.json`, which `lib/trailData.ts` fetches for the A.T. elevation ribbon; `trail_graph_elevation.json`, the per-edge climb a day hike's card prices its ± elevation and ≈ walking time from ([#1011](https://github.com/OurHike/OurHike/issues/1011)); and `trail_graph_profile.json`, the dense per-edge profile a ribbon on a followed day hike draws from ([#1045](https://github.com/OurHike/OurHike/issues/1045)). The last two answer different questions and are not interchangeable — the scalars are the sanctioned total a card prices from, the profile is the shape of the ground. Tick it for any release those should be current in. Leaving it off is not a dry run for them: the manifest merge is additive-only, so all three keep whatever the last elevation run published while everything else moves on.
...
**And the same thing on the way out: deleting a preview and its URL going quiet are not the same event either.** Measured 2026-08-25 on `pr-1003` — the teardown listed its one deployment, deleted it with `force=true`, Cloudflare answered `"success": true`, and 32 minutes later `https://pr-1003.ourhike-preview.pages.dev/` was still serving that build's own files (`/assets/main-BTYohYcr.js`, 816,086 bytes). Cloudflare documents no retention or propagation window for a deleted deployment's branch alias, so what happens after that is not known here. The comment a closed pull request gets now reports what the URL actually answered rather than asserting it stopped; see [#1004](https://github.com/OurHike/OurHike/issues/1004).

**Cost: none at this size.** The free plan places no limit on how many preview deployments a project keeps — "You can have an unlimited number of preview deployments active on your project at a time", [Pages limits](https://developers.cloudflare.com/pages/platform/limits/), read 2026-08-25 — and static requests and bandwidth are not metered. The limit that *would* bite — 500 builds a month, one at a time — applies only to Cloudflare's own builders, and this workflow does not use them: it builds in Actions and uploads the finished directory, so nothing queues behind anyone else's build.
...
**Clearing the backlog needs this token, and is the one thing here no workflow does.** The teardown on the closed path selects deployments by the alias of the pull request in front of it, so it never looks at anything orphaned earlier — and it could not list anything at all between 19 and 25 August, first because its manifest would not parse and then because it sent a `per_page` the API refuses ([#990](https://github.com/OurHike/OurHike/issues/990), [#1001](https://github.com/OurHike/OurHike/issues/1001)). `scripts/sweep-pages-previews.sh` is the one-off that reaches backwards. It wants the three settings above in the environment plus a `GITHUB_TOKEN` to read pull request states, it prints its plan and deletes nothing until told to, and `--delete` on its own removes one alias — because of the paragraph about deleted and quiet being different events. Its `--help` is the full story.
...
- **Email** — enabled by default in a new project, and **that is not the same as working**: a hiker gets a numeric code in an email, and nothing sends that email until 4.3c is done, nor puts a code in it until 4.3d is. Both projects.
...
**4.3d The code in the email, on both projects.** `screens/EmailSignIn.tsx` asks for a **code, of whatever length this project is set to send** — not a link (#279: a link tapped in a mail app opens the browser, not the installed app, and the session lands where the app cannot see it). Whether the email carries a link or a code is the template, and there are two of them: a returning address gets **Magic Link**, a new address gets **Confirm sign up**, and `verifyOtp({ type: 'email' })` accepts either token. A code in one template and a link in the other is a sign-in that works for everyone but a new hiker.

**Two things here have cost a working sign-in, both found on 2026-09-21 (#1600), both after a merge, both from a hiker rather than from a check.**

*The length.* **Authentication → Providers → Email → Email OTP Expiration** sits beside a length setting, and Supabase allows **6 to 10**. UA was on **8** while `EmailSignIn.tsx` hardcoded 6 and capped its field at 7 characters — so a browser silently dropped the last digit of every code, every attempt was refused, and the screen told the hiker to check their six. Nobody can get that right by being careful. The screen takes the whole 6–10 range now and names no number, so any setting works; `check_supabase_config.py` is what asserts the project stays inside it. **Leave the length alone unless you have a reason, and if you change it, run the check.**

*The link.* Adding `{{ .Token }}` to Supabase's stock template is **not** enough — a template carrying `{{ .ConfirmationURL }}` as well makes Supabase send a magic link whatever else is in it. **Delete the link.** The check now fails a template carrying both, which it did not when #1576 shipped.
...
**Since [#397](https://github.com/OurHike/OurHike/issues/397) that risk was deferred rather than accepted — v1 shipped Google alone — and since [#279](https://github.com/OurHike/OurHike/issues/279) landed with [#1572](https://github.com/OurHike/OurHike/issues/1572) it is gone rather than deferred: the email path is an emailed **numeric code**, `EmailSignIn.tsx` has no password mode, and `lib/auth.ts` has no `signInWithPassword` call.** No OurHike build has ever shipped a password to a hiker. The advisor line stays flagged and stays moot: there is no password for a breached one to be reused as.
...
4. **Point the client at it** by setting the `API_BASE_URL` repository variable, and add its origin to Supabase's allowed redirect URLs (4.3b). The variable is read by `pages.yml`, which bakes it into the next build, so the app reaches the backend from that deploy onward rather than immediately.
5. **Confirm it, in this order**, because each step tells you something the previous one cannot:
   ```
   curl -sS https://<service>/health          # {"status":"ok"} - the app is up
...
**The image builds and runs — measured 2026-09-09 (#600), and this paragraph said it never had.** `backend/Dockerfile` unmodified, built against Docker 29.3.1 and run against a real Postgres 16 and an empty database. What that establishes, each of which was previously "should work": the image builds (12 layers, 406 MB on disk); a missing `SUPABASE_URL` raises a pydantic error naming the field at startup rather than booting on an empty credential; `PORT` is honoured — probed at 10000, and the `ENV PORT=8000` fallback when it is unset, which is the one line [backend/HOSTING.md](backend/HOSTING.md) rests portability on; `/health` answers 200 and `/openapi.json` serves; booting creates **no** tables, so the Dockerfile's promise that a migration never fires unattended holds; and after `alembic upgrade head` inside the container, every table the migration creates has row-level security on — step 5a's check, answered against the migration rather than assumed (18 of 18, re-measured 2026-09-09 against the hardened image; it was 16 of 16 hours earlier, which is the count moving with the migrations rather than the answer changing).

**What that run did not cover, so still budget for the first deploy to surface something.** It used a direct connection to a local Postgres, not Supabase's transaction pooler ([#95](https://github.com/OurHike/OurHike/issues/95) is that gap and stays open), and no real Supabase project — so RLS is confirmed as *what the migration does*, not yet as what production carries. The sandbox re-terminates TLS, so its base image was given the proxy's CA for `pip` to reach PyPI; every other layer is the committed file. Two findings worth having in advance: **deploy-before-migrate** looks like `HTTP 500` with `UndefinedTable: relation "closures" does not exist` on every database endpoint, not a crash — which is step 3 above being skipped, and is diagnosable in a second if you have read this sentence and slow otherwise. And **the container no longer runs as root** — that sentence stood here for a few hours and is now false: [#1358](https://github.com/OurHike/OurHike/issues/1358) added a `USER` and the whole matrix above was re-run against the changed image, including `alembic upgrade head` as the unprivileged account, which is the step a permissions mistake would have shown up in. The same change stopped the image shipping 81 `.pyc` files compiled on whoever ran the build: `.dockerignore`'s `__pycache__/` matched only the top level, because Docker compares each pattern against the whole path rather than against every segment the way `.gitignore` does. A clean CI checkout has no `__pycache__` to copy, which is why nothing had noticed. See [backend/README.md](backend/README.md) for the reasoning behind each choice.
