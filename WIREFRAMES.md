...
The panel named categories the map draws as pins and drew none of them, so the one screen whose whole job is to teach "this shape means water" never showed the shape. The confidence split was the sharper loss: the legend printed "Unverified" in words while the map said the same thing with a **broken rim**, and nothing on screen joined the two — a hiker who has learned that dashed means nobody has checked learned it somewhere else. The icons come from `client/src/map/MapIcon.tsx`, which draws from the map's own `pinGeometry()`, `GLYPHS`, `POI_COLORS` and `RIM_DASHES` rather than from a second drawing of a pin. That constraint is the point of the file: a legend that approximates the map is worse than one that shows nothing, because it teaches a symbol the map does not use. So a closure shows the barred band (it is a line, not a pin), a serious warning the hollow hazard triangle, and an unverified waypoint the broken rim. The one place it parts company with the map is **size** — the warning pin is 44px there against a waypoint's 38 because it has to win a glance across a moving screen, and in a key read a row at a time a taller row buys no urgency and costs the grid its alignment. What carries the recognition instead is what carries it on the map: the only outline among solids.
