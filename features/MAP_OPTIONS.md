...
`App.tsx` now fetches them on first run when a data source is configured and the phone is online, quietly: a failure leaves exactly the empty map it would have had, and the Downloads screen still reports errors for the download a hiker actually asks for. Reading what is already stored stays unconditional and independent of that fetch — an unconfigured build and a phone with no signal both still have last time's download, and gating the *read* on connectivity would leave a hiker on a ridge looking at a map with no trail. The corridor archive is untouched and stays behind its button; it is hundreds of megabytes of someone's data allowance and that remains their call.
