...
- **Default: every trail line is one red** — `client/src/lib/blaze.ts`'s `PLAIN_TRAIL_COLOR`, an alias of the palette's Red rather than a new hue, so the closed palette above is exactly as closed as it was. Red light still wins over it, as it wins over the hues. _(Amended 2026-09-18, [#1588](https://github.com/OurHike/OurHike/issues/1588): under that default the red is a vocabulary — a trail without a pill is the red tinted to 45% over the sheet's paper, thinner and dashed, with no casing; the A.T. and the Long Path are the red at full strength, solid, with no casing. The tint is derived from the palette's Red per sheet, `client/src/map/style.ts`'s `contextTrailColor`, not a new member. With the switch on the map draws exactly what this document designed, casings and all.)_
