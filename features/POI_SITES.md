...
This overlaps the chips below on type and distance, deliberately. The chip is a *control* — its job is to lead somewhere; the sentence is what a hiker skims before deciding whether to tap anything. *(Amended 2026-08-16 — the overlap is now with the meta line rather than with the chip, which since §5's last amendment shows no words at all. The point stands and is the reason that line took the words: a sentence you skim and a control you tap are different things, and the sentence is what answers "is there a privy" without a tap.)*
...
| [#1682](https://github.com/OurHike/OurHike/issues/1682) | 17 px badges against the 26 px slim pin, upper right | **14.0 px** | **9.1 px at every count** | 46 / 56 / 58 px |
...
**And it costs rasterising time**, which is worth writing down because it is the one cost that was found by CI rather than by looking. The images are 2.2× the pixels they were before badges, and building all 46 of them is a few hundred milliseconds — enough, unoptimised, to time out tests that mount a map. Three things pay for it: the rasteriser skips any pixel whose samples cannot reach the pin or a badge (about half of a three-member image is empty corner); `buildPoiIcons` is built once per process rather than once per map, since every trip to the More tab and back was paying the whole bill again for an answer that cannot change; and since [#857](https://github.com/OurHike/OurHike/issues/857) the build runs in a worker rather than on the UI thread ([`client/src/map/poiIconImages.ts`](../client/src/map/poiIconImages.ts)), with the plain call kept as the fallback where there is no `Worker`. "A few hundred milliseconds" was measured on CI hardware; on a 4×-throttled phone profile it was **2,521 ms**, measured 2026-08-20, which is what made where it ran worth deciding rather than assuming. Nothing asks for the images at all until there is a waypoint to draw — the style's `match` arms are reached by a feature or not at all, so a map with an empty POI source needs none of the 46.

> **Amended 2026-09-26 ([#1682](https://github.com/OurHike/OurHike/issues/1682)) — 17 px badges on a slimmer pin.** The waypoint pin is now drawn 26 px across inside its 38 px footprint, with a paper hairline rather than the cream halo and dark edge, so the badges moved with it: the member's accent disc, its glyph in paper, a paper ring, and no dark hairline, tangent to the pin's disc so they never cover the anchor's glyph. Colour-only pips were built first; the maintainer, shown them on the real map: *"When you put the icon in the upper right, make it the icon, not just the pin."* The size was chosen from the app built three ways and photographed on Limestone Spring Shelter (CT): 14 px (the coin's proportion, a 7.5 px glyph — the size this section records as too quiet), **17 px** (a 9.1 px glyph), and 21 px (today's badge unchanged, nearly the size of the pin it rides). The anchor still outranks its members, 14.0 px against 9.1, and the site image is smaller than it was — 46 / 56 / 58 px for one to three members, against 58 / 70 / 72 — so a site pin now claims less collision room than before. At 17 px three badges fan 53° either side of the corner.
...
**Amended 2026-08-14 by [#711](https://github.com/OurHike/OurHike/issues/711): scroll is reachable, and reachable is not discoverable.** A hiker reported not being able to see all the parts of a site, and the paragraph above is why. It weighed horizontal scroll against widening and wrapping, decided correctly between those three, and then treated "the strip scrolls" as though it answered the question — but the strip also sets `scrollbar-width: none` and `::-webkit-scrollbar { display: none }`, deliberately, because a scrollbar would eat a chip's width on a thumb-dragged strip. So **nothing on screen says there is anywhere to scroll to.** Measured in Chromium 1194 at the card's real width: a three-part site asks for 406 px of a 240 px strip and a five-part one for 691. Two chips fit; the rest are reachable only by a drag the hiker has no reason to attempt. That is #524's own failure mode one layer up — not a pin the collision engine dropped, but a part of the place with no gesture leading to it — and it lands on the water and privy questions §1 opens by calling the ones a hiker actually asks.

What changed is the chip, not the strip: an unselected chip is now its pin alone, at the touch target and no wider, and only the part being read spells itself out. The words stay in the DOM behind `visually-hidden`, so a screen reader hears exactly what it heard before. Measured the same way — 3 parts 406→**240** (fits), 3 parts with the campsite open 407→268, 5 parts 691→**289**, 5 parts with the campsite open 692→364. **It does not close every case and should not be read as though it does**, which is why the numbers are here rather than a word like "fixed": five parts still scrolls. What changes is that every pin is on screen or about one chip from the edge instead of 450 px past it. Three costs, all real: the row now shuffles under the thumb when tapped, where the inset current-chip ring exists precisely so that marking a chip does not resize it; an unselected chip is a symbol the hiker has to recognise; and one distance is legible at a time instead of two. Height does not move, which is the constraint the paragraph above was right to protect. The whole of the residue is the selected chip's own distance — `Campsite · 181 ft` is 172 px of that 364 — so dropping it and keeping the category alone would fit every case at 289 px flat; the maintainer chose to keep the distance, on the grounds that it is the half of the chip a hiker is asking for.

**Amended 2026-08-16: the strip is pins only, and the words are the line under it.** The paragraph above ends on a choice between the selected chip's distance and a strip that fits, and treats them as exclusive. They are not — the distance can be kept and moved. **The chip strip carries nothing but pins now, the part being read included, and the meta line under it says that part's category and how far it is from the pin: `Privy · mi 1913.4 · 131 ft away`.** That line already carried the category, and the heading above it already carried the part's name, so the only fact that had to go anywhere is the distance.

Measured the same way — Chromium 1194, the card's real width, `PoiCard.test.tsx`'s fixtures — but as chip boxes plus gaps rather than `scrollWidth`, which floors at the container and so reports a fitting strip and a full one with the same number:

| site | before #711 | after #711 | now | has |
|---|---|---|---|---|
| 3 parts, as it opens | 406 | 180 | **140** | 240 |
| 3 parts, campsite open | 407 | 244 | **140** | 240 |
| 5 parts, as it opens | 691 | 276 | **236** | 240 |
| 5 parts, campsite open | 692 | 348 | **236** | 240 |

So **every site size the trail has now fits**, and the strip no longer changes shape when a chip is tapped — the fixed geometry the inset current-chip ring exists to protect, which #711 spent and this buys back. Two things are unchanged and should not be read as fixed by this: a chip is still a symbol a hiker has to recognise until they tap it, and **six parts would ask 284 px and scroll**, undiscoverably, exactly as before. Six is the number to re-run this against if [#529](https://github.com/OurHike/OurHike/issues/529)'s water gap closes and sites grow; five is the largest that exists today.

One cost, in the other direction: `.poi-card__meta` now wraps, because a shelter that is *not* the pin — reachable since [#607](https://github.com/OurHike/OurHike/issues/607)/[#609](https://github.com/OurHike/OurHike/issues/609) — carries four facts, and `Shelter · mi 2189.4 · Sleeps 8 · 131 ft away` measures 240 px against the body's 240. That is one wrapped line on one part of one site shape, against a strip row that would have been permanent on every five-part site; and without the wrap the flex line shrinks its items instead, breaking `mi 2189.4` across two lines at the space. `usePinAnchor` already re-measures on the selected part, so the height change is handled rather than avoided.

@unvalidated, inherited from #711 rather than introduced here: nobody has tapped a 44 px icon-only chip with a gloved thumb in sun. What would settle it is the field test [HIKER_SAFETY.md §5](HIKER_SAFETY.md) declines to guess at. What this change adds to that exposure is one more chip per card.
...
- **~~The lanes ribbon has a dead control.~~ Closed, and not by this.** When this was written `WaypointLanes.tsx` rendered each cluster as a `<button>` with no `onClick` — WIREFRAMES.md §1.4's count pill, built but inert — and sites were offered as the thing it should open. [#1054](https://github.com/OurHike/OurHike/issues/1054) replaced the lanes with the next-up rail, which closed it a different way: `chrome/NextUpRail.tsx` wires `onClick={() => onSelectPoi(point.id)}` and makes `onSelectPoi` a **required** prop, on the reasoning that "an optional handler would let an inert rendering back in with a green typecheck." A card opens the same waypoint card a map pin does. Kept here rather than deleted because the sketch's other bullets are argued against this one.
