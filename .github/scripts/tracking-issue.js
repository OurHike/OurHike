...
// green again. Four scheduled workflows share this: check-deployment.yml,
// check-deployed-app.yml, check-upstream-freshness.yml and
// smoke-published.yml.
//
...
async function trackingIssue({ github, context, core }, options) {
  const { label, title, healthy, allClear, checkedAt, keys = [], render } = options
  ...
  if (existing) {
    ...
    return { action: 'updated', number: existing.number, firstSeen }
  }
