...
export const alt =
  'Either the day-hike builder in a wide browser window holding a walk - a narrow panel down the left opening with the three-stop rail (Day hike ✓, Route lit, Details), then "Your route" with Distance, Climb and Walking figures, the route order with a stop row for Fingerboard Shelter beneath them and a two-column grid of map-label toggles at its foot, the map filling the whole right of the window with the route cased dark, and the builder bar with Cancel, the Shape row, its actions and the "‹ Hike" / "Use this route ›" foot along the bottom of the map - or the same rail still waiting for a first tap, where the search found no such shelter or no tap landed on a trail; or, where this build has no junction graph, step 1 with its sentence saying the trail network is not on the phone in place of the map door'
