...
**The deployed set is the `AUTH_PROVIDERS` repository variable, and #1572 makes it `google,github,email`** - as the *last* step, after GitHub's OAuth app exists, custom SMTP is configured and both email templates carry the code, and after the config check has read all of that back. The code default stays `google`, the one provider a fresh project can complete with a Cloud Console client and nothing else. Apple is deferred to v2 ([#92](https://github.com/OurHike/OurHike/issues/92)).
