...
**Built today:** the Commons source (`pipeline/fetch_poi_images.py`, the `photo_*` properties, the card's credit line) with its images served from our own bucket (#362), ATC's facility photos (`fetch_atc_photos.py`, rung 3), the **private half of "Your own photo"** — capture from the card with a keep-or-discard review (#571), several per place with a sticky choice of which one the card shows (#575), with the archive-honesty behaviour #573 asked for (see "not a photo archive" below) — and **"Sharing", end to end**: the backend store with its caps, recorded licence and attribution request (#576), rendered on the card as rung 2 with rung 1 above it (#578); the share sheet with its plain-words terms, the two-hour cooling-off window, and a withdrawal whose meaning the strip states truthfully by phase (#577); and the moderation surface — pin up to three, unpin, one-glance review, takedown, and the report-this-photo path that keeps the rolling twelve safe without pre-approval (#579). **Screening is built on both paths (#836, #837):** the pipeline face-checks every Commons photo at fetch time and holds a flagged one from export and bucket until a person records a verdict, and the share sheet runs an on-device nudity-and-face check whose finding rides the share as the flag the queue already sorts and holds by — with the pipeline's nudity half held open as #850, a dependency decision.
...
**A takedown outlives the row it was recorded on** (#1551, maintainer's decision 2026-09-17). A moderator's dismissal sets `status`, `dismissed_at` and `dismissed_by` on the photo row, and that row is not a record: a re-share upserts over it, because a replacement is a different photograph, and a withdrawal deletes it, because that is the contributor's own act. So every takedown is also written to a ledger, `poi_photo_dismissals`, keyed by the same (place, contributor) pair as the store, with who decided, when, and what the queue showed them as they did. The queue sorts a previously-taken-down contributor's next share of that place ahead of the unreported tail, below any live report, and shows the count. Nothing a hiker can do reaches the ledger. It is a moderation record, like `reports.dismissed_by`, not a contribution: account deletion leaves it, and the export hands the hiker their own rows from it. A count on the photo row was considered and refused because the row's deletion would have taken it with it.
...
- **Fetch time, first** — [#836 — Run a face-and-nudity check over the Commons fetch, where a stranger's self-portrait can become a town's illustration](https://github.com/OurHike/OurHike/issues/836). The Commons rung is the path with live exposure and no human in it (the `David Rudmin` case above), a workstation can carry a heavy model, and a flagged file waits for a human look before export — no hiker is present to be refused, so holding is not gating. **Built 2026-08-20, faces first:** every stored photo is screened at download time (`pipeline/lib/photo_screen.py`, OpenCV's stock Haar cascade — in the wheel, no model download), a flagged photo is held out of the export *and* out of the public bucket until `reference/photo_screen_decisions.json` records a person's verdict, and `review_flagged_photos.py` is the one-person review surface that records it. The nudity half did not ship with it, on purpose: no self-contained nudity detector exists the way the cascade does, so it is a dependency decision — [#850 — The pipeline's Commons screen checks faces but not nudity, because a nudity model means a new heavyweight dependency](https://github.com/OurHike/OurHike/issues/850) holds that gap, with what bounds it meanwhile.
- **Share time** — [#837 — On-device screening at the share sheet: friction and a queue flag, never a refusal](https://github.com/OurHike/OurHike/issues/837). Plain words at the sheet about what was detected, before anything irrevocable, and a priority flag that sorts the moderation queue. Its design question — whether a machine-flagged share waits for one human glance before the rolling 12 — **was answered by the maintainer-adopted queue mockups and is built (2026-08-20): a nudity flag holds, a face flag only sorts.** Held is not refused: the share succeeds, the queue shows it first with "nothing is published while it waits here", and one moderator glance releases it. **The detector itself landed the same day**: nsfwjs's MobileNetV2 (nudity) and BlazeFace (faces) over one TensorFlow.js runtime, run when the share sheet opens, entirely on the phone. A finding adds one screening step — the adopted mockups' copy, sentence for sentence: faces get the fact and the same two buttons, nudity gets "Send it for review" and a preview hidden until tapped — and rides the queued share as its `flagged` value. Every failure path (offline before the engine chunk arrived, no WebGL, a decode error) resolves to the same null as "nothing found", so absence of the check costs exactly what pre-#837 shares cost: report-driven moderation. Measured costs (2026-08-20 build): the engine is one lazy 1,125 KB chunk (286 KB gzip) excluded from the app-shell precache by name — the precache stayed at 8.48 MB — and the vendored weights are ~3.2 MB fetched on first use only (nsfwjs 2.7 MB extracted from its own npm package, BlazeFace 0.47 MB from the URL its package would have fetched); `scripts/check-build-output.mjs` fails any build where either leaks into the precache. Both thresholds are `@unvalidated` priors leaning toward the human glance; the shared-photo corpus that would tune them does not exist yet.
...
Nor can it be shrunk away: the slot is 240 CSS pixels, so a DPR-2 phone needs ~480 device pixels to look crisp. 640px is already close to right-sized, and 320px is visibly soft. **~140 MB is roughly the floor for crisp offline photos at 3,000 POIs.**

> **Corrected 2026-09-15 (#1495): the slot is 240, not the 264 this line said for a year, and the DPR-2 figure is therefore 480 rather than 528.** 264 is the *card's* width; the media box sits inside `.poi-card__body`'s 12px padding, so the photograph gets 264 &minus; 24. The line was right when it was written and stopped being right at [#941](https://github.com/OurHike/OurHike/issues/941), which moved the photograph off the card's top edge and down inside the body padding &mdash; the same change whose CSS comment explains why the media box needs its own border radius now. Nothing re-derived the slot afterwards.
>
> **Neither conclusion moves.** 640px is still the right stored edge and 320px is still visibly soft; the correction makes 640 slightly *more* headroom than claimed, not less. What the real number now carries is a decision it could not carry while it was wrong: `MIN_HERO_PHOTO_WIDTH` in `client/src/lib/poiPhotos.ts` is 240 &times; 2, the width at or above which a photograph may fill the card's box instead of being drawn at its own size inside it.

> **Superseded 2026-08-09 by measurement, and in both directions — see [PHOTO_DOWNLOADS.md](PHOTO_DOWNLOADS.md)'s Finding 1.** The table above assumes every one of ~3,000 POIs carries a photo and that each is ~47 KB. Neither survived contact with real files. **489 POIs have a photo**, not 3,000 (the ATC coverage measured above: 270 shelters + 219 campsites), and a real 640px rendering is **~145 KB**, not 47 KB — which is what "treat them as a floor" was warning about. The two errors do not cancel: the whole corpus is **~71 MB today** and **~189 MB after [#471](https://github.com/OurHike/OurHike/issues/471)** ships every ATC photo, against 607 MB for the terrain package alone.
>
> So the conclusion in this section is the one that has to move: **bundling everything is not out, it is about an eighth of the terrain file the app already ships.** What stays true is everything the section says about *shape* — the 640px slot cannot be shrunk, metadata is a rounding error, and an offline pack is opt-in with a measured size on the button. Points 1–4 below are unchanged except that point 4's PMTiles reasoning is answered rather than open: content-addressing already gives per-photo addressability, so photos stay individual objects and never become one indexed lump.
...
- **Offline delivery — now spiked separately, in [PHOTO_DOWNLOADS.md](PHOTO_DOWNLOADS.md).** Mirroring into R2 (above) fixes reliability, not offline: the card still fetches a URL when it renders, and offline it falls back down the ladder. Photos are enhancement and never safety-relevant, so that is an acceptable resting state. What that spike changes is the size budget's premise rather than this document's design — measured, the whole corpus is **489 POIs and ~71 MB**, not 3,000 POIs and ~137 MB, so "bundling everything is out" was concluded against an estimate wrong in both of its factors. It owns the download verbs, the hike and section scopes, and the automatic-fetch rule from here on; this document keeps the ladder, the sources and the bytes.
...
- ~~**Whether an irrevocable licence needs a cooling-off period.**~~ **Decided with the share flow (#577, 2026-08-20), in the shape this bullet sketched and the maintainer-adopted mockups drew: two hours.** A shared photo goes public two hours after its bytes land (`COOLING_OFF_HOURS`, app/routers/poi_photos.py); inside the window a withdrawal is a true undo — nobody ever saw the photo, so no copy exists to be beyond reach — and the sheet and the strip both say which side of that line a share is on. Two hours is a judgement, not a measurement: the regret this exists for is minutes-scale ("regrets it ten minutes later"), photos are never safety-relevant so nothing time-critical is delayed, and a gallery two hours behind the newest snapshot is a lag no hiker's decision turns on. `@unvalidated` in the one direction worth watching: whether real regrets cluster past two hours, which only real shares can say.
