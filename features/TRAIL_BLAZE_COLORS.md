...
**Where this landed, and where it stands now (2026-08-03).** The secondary cue shipped as dash style, then came back out. Per-blaze dash rhythms made the map's own subject unreadable: the AT centerline's blaze is near-white, so the gaps in its dash - dark casing showing through - were the part that read, and the trail a hiker was standing on looked like a dotted grey-and-white thread. Lines are now solid, and **width** carries the hue-independent channel instead: a system's through-route is drawn markedly wider than the side trails hanging off it, keyed off the pipeline's `source` attribute (see WIREFRAMES.md §3). With the AT the only through-route on today's map, that answers "which line is the AT" with hue removed entirely, which is the question this section's own accessibility argument cares most about. It answers it for exactly as long as that stays true: the tier is a role rather than a name for the AT, the NYNJTC alone maintains several trail systems, and a second through-route on the map turns the question width answers into "through-route or spur". *(The dash came back once more as a dot rhythm on every line outside the taken system — [#1283](https://github.com/OurHike/OurHike/issues/1283), 2026-09-08 — and went again on 2026-09-10 in [#1374](https://github.com/OurHike/OurHike/pull/1374), on the maintainer's read of a z13 frame: "the dashes are distracting". Lines are solid; the paragraph above stands.)*

It does **not** answer "is this spur yellow or orange" - those two are now separable by hue alone, and 26 features in today's data are affected. The tap sheet naming the blaze remains the honest fallback there. **Dash style is not coming back as the second cue** (decided 2026-08-03, WIREFRAMES.md §3): no dashed trail lines at all, closures excepted, since a dash over a dark casing reads as its gaps - which is what took the first attempt out. So the open work here is narrower than "a secondary cue" and should be read that way: a *non-dash* one, a casing weight or a label at high zoom, for the warm hues and eventually for two through-routes.
...
## The hues became a switch (added 2026-09-17, [#1575](https://github.com/OurHike/OurHike/issues/1575))

Design point 2 above — "rendering is a data-driven MapLibre style expression" — still holds, and is now one of three answers `client/src/map/style.ts`'s `blazeLineColor` gives for every trail source at once. The maintainer, 2026-09-17: *"Showing the blaze color can be distracting and feel like I'm living in a rainbow. Provide a switch for the user to 'Show blaze colors' in the legend. Maybe the default color should be a red line, like the nynjtc has on their maps. Its easily readible."*

- **Default: every trail line is one red** — `client/src/lib/blaze.ts`'s `PLAIN_TRAIL_COLOR`, an alias of the palette's Red rather than a new hue, so the closed palette above is exactly as closed as it was. Red light still wins over it, as it wins over the hues. _(Amended 2026-09-18, [#1588](https://github.com/OurHike/OurHike/issues/1588), and again 2026-09-20, [#1597](https://github.com/OurHike/OurHike/issues/1597): under that default the red is a vocabulary — a trail without a pill is thinner and dashed, the A.T. and the Long Path are full width and solid, and **both are the same `PLAIN_TRAIL_COLOR`**, with no casing on either. #1588 tinted the context trails to 45% over the sheet's paper; the maintainer read that as pink and asked for the A.T.'s own red, so the vocabulary is now entirely width and dash and the palette gained nothing at any point. With the switch on the map draws exactly what this document designed, casings and all.)_
- **The switch** is the legend's "Blaze colors" row (WIREFRAMES.md §2), a stored and synced preference — `blaze_colors_shown` in [IDENTITY_AND_PRIVACY.md](IDENTITY_AND_PRIVACY.md)'s model. On, the map draws exactly what this document designed.
- **Only the map's lines change.** The tapped line's sheet, the legend's "Trails in view" swatches, the through-route badge's chip and the day hike card's legs keep the blaze hue — the maintainer, the same day: *"Make sure that the blaze color still shows on the 'Planned Route'. Changing the color option should only affect the map itself, not the other options."* This document's accessibility argument reads differently under the default as a result: with hue off the map, **width** is the only channel at a junction and the tap sheet is the only key on the map itself; the legend's rows are the key for the trails the map can name.
- **Two things this reopens, both `@unvalidated`:** whether a junction of two red lines reads outdoors as well as two hues did (the outdoor pass, [#105](https://github.com/OurHike/OurHike/issues/105)), and whether the default should be the hues after all. The default is one line in `DEFAULT_PREFERENCES`, and the maintainer's "maybe" is recorded here as the decision it was made on.
