...
So **six** of the eight external sources now carry `reaches_hikers: true` and ship: `oprhp_trails`, `oprhp_trail_closures`, `nynjtc_long_path`, `nynjtc_highlands_trail`, `mohonk_trails` and `dec_hiking_trails` — the last three added since this paragraph was written, by [#992](https://github.com/OurHike/OurHike/issues/992) and [#1019](https://github.com/OurHike/OurHike/issues/1019), each on the maintainer's authorisation because none of those three stewards states terms. The closure layer left the held-back group under [#964](https://github.com/OurHike/OurHike/issues/964), which derives it onto the trail lines. The two that stay false — OPRHP's facilities and park polygons — are false for a different reason: they are registered and **nothing exports them yet**, which is `reaches_hikers`'s other meaning and worth keeping distinct from a licence hold. OPRHP's required attribution rides in `client/src/map/credits.ts` and appears in the map corner whenever their lines are drawn; `export_sources.py` names both stewards on the sources screen. The [#771](https://github.com/OurHike/OurHike/issues/771) spike was this data's first consumer and the export is its second.
