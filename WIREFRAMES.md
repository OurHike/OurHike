...
- The empty-state sentences, the "turn Verified? off" sentence, the below-the-pin-floor sentence and the drop summary are all still decided by `computeLegendContents` alone. "Nothing on this part of the map yet" still appears, over eight rows of zero.
- **Safety rows are not padded.** `closure` and `serious-warning` are not in `HIDEABLE_TYPES`, have no switch to reach, and a standing "Closure 0" would be this panel making a claim about closures that nothing asked for.
