...
`.github/workflows/settings-check.yml` runs the settings suite. Its `manifest` job runs on every PR; its `configured` job deliberately does not, because GitHub passes no secrets to a fork's PR run and the job would fail for every outside contributor for a reason none of them could fix. This is the one gating workflow with **no** `merge_group:` trigger, so the settings suite does not gate a merge queue. That is on evidence rather than principle: while these triggers were being added, this workflow reported no status at all on ordinary pull requests - `action_required` with zero jobs - and removing the trigger again did not stop it. It is the only workflow here that both runs on `pull_request` and reads the secrets context. BRANCHING.md holds the measurements, what is ruled out, and what is still only a hypothesis.
