...
- **Off the path** - physically wandered off the trail line. This reuses [MAP_OPTIONS.md](MAP_OPTIONS.md)'s snap-to-segment math exactly: the same `ST_LineLocatePoint`-based distance-to-centerline computation, just run continuously against live GPS instead of once against a tap. If the live position's distance from the nearest trail line exceeds a threshold, the hiker is probably off-trail.
