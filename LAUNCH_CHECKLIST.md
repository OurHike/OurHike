...
**1.6 Publish — now a CI workflow, not a local script.** Dispatch **"Publish vector data"** (Actions tab → workflow_dispatch) with `publish` ticked. Leaving it unticked does a dry run: builds and quality-checks everything without uploading, useful for checking upstream still parses. `include_elevation` adds ~25 min and builds two artifacts the app **does** read: `elevation_profile.json`, which `lib/trailData.ts` fetches for the A.T. elevation ribbon, and `trail_graph_elevation.json`, the per-edge climb a day hike's card prices its ± elevation and ≈ walking time from ([#1011](https://github.com/OurHike/OurHike/issues/1011)). Tick it for any release those should be current in. Leaving it off is not a dry run for them: the manifest merge is additive-only, so both keep whatever the last elevation run published while everything else moves on.
