...
export const alt =
  'The map screen as a new hiker first opens it after the entry cards: the identity plate at the top, and below it the whole Appalachian Trail as a single fine dark line from Georgia to Maine fitted to the screen over the basemap, with the other organizations’ trails as fine threads of the same weight around its New York miles, no pins, one paper badge reading “Appalachian Trail” with the ATC mark on the line, a 100 mi scale bar, and no elevation strip along the foot since nothing is taken (#1374 review)'
