...
- The download window actually fetches data instead of saying "data source not configured" (that message means `DATA_BASE_URL` didn't make it into the build). It opens from the link under the background picker — in the legend, or in Settings under the More tab.
