...
- **Email.** One way in, decided 2026-09-17 ([#279](https://github.com/OurHike/OurHike/issues/279), [#1572](https://github.com/OurHike/OurHike/issues/1572)): **a numeric code, emailed and typed into the app.** Not a link and not a password, and the two earlier versions of this bullet are worth keeping in one sentence each, because each was replaced for a reason that still holds.
...
- `client/src/chrome/AccountButton.tsx` and `chrome/SignInWindow.tsx` - the door and the room (#1596). Signing in is one tap from every screen, and **where that tap is depends on the layout, because the layouts differ in how many screens they draw at once**. On a phone the button sits in the map header's action cluster beside Legend and Search, and in Today's own chrome - one screen is up at a time, so a hiker sees one button. Above the 900px breakpoint Today and the map are side by side and both of those were on the page together, so the button moves to the top-left corner of the desktop sidebar (`chrome/TabBar.tsx`) and `desktop.css` hides the other two. The sidebar is the one piece of chrome both screens share, which is what makes one button there one button on the page. It was three taps inside More until 2026-09-20. The window is docked over whatever is behind it rather than replacing it, which matters most on the contribution path, where a `flowScreen` used to take away the map the report was about. **It is a door, not a wall**: nothing is gated by it, and #393's warning is about gating *browsing*, which would move every reader onto the metered line.
...
**The provider set is code**: `ENABLED_PROVIDERS` in `client/src/lib/supabase.ts`, which since [#1572](https://github.com/OurHike/OurHike/issues/1572) names `github`, `google` and `email` - **in that order, which is the order the buttons are drawn in**. GitHub leads on the maintainer's instruction, 2026-09-20: "we are an open source and open data project". The four possible doors do not cost the same to switch on - Google and GitHub each need a free OAuth registration, Apple the $99/yr membership, email a sender - and a button whose credentials do not exist reaches an error page rather than an account. `SignInPrompt` still defaults to all four, so the wireframe's answer (plus GitHub) stays the component's; narrowing is what that constant does.
...
**It was a repository variable, `AUTH_PROVIDERS`, until 2026-09-20**, and the removal is worth keeping because the reasoning behind the variable was sound and the mechanism was not. Sound: a build must not offer a button whose credentials do not exist. Not sound, measured the day it went: `pages.yml`, `ua.yml` and `pr-preview.yml` all read the *same* variable, so it never gave production, UA and the previews different sets; production builds from a `v*` tag, so it was never a live switch either; an unset variable meant Google alone with nothing naming the cause, which cost the maintainer half an hour hunting for buttons the code already had; and it put the list where no test could reach, so `.github/tests/test_privacy_policy.py` was checking the privacy policy against a set the deployed build did not use. What it did buy was one incident lever, a dispatch of `pages.yml` dropping a provider without a merge, and that was traded away deliberately. Apple stays deferred to v2 ([#92](https://github.com/OurHike/OurHike/issues/92)).

**A refusal at Google or GitHub says so, since [#1573](https://github.com/OurHike/OurHike/issues/1573).** Those two doors are a full off-origin navigation, so the failure lands back on `/app/` as a fresh page load with `#error=access_denied&error_code=…&error_description=…` on the URL and no component left alive to have been told. supabase-js reads that inside `GoTrueClient._initialize()`, which this app does not await and which fires no `onAuthStateChange` - so a hiker who tapped Cancel used to land on the map, signed out, with the window shut and nothing anywhere saying what had happened. `client/src/lib/authRefusal.ts` reads it off both the fragment and the query at boot, maps it through `signInMessage`, and `App.tsx` re-opens the ask with the sentence above the doors; the three error keys come back off the address bar so a reload cannot repeat it. That closes the last silent path in sign-in, which is [#397](https://github.com/OurHike/OurHike/issues/397)'s acceptance - **every** refusal is a sentence on screen, not only the email ones. What the two new sentences say is `@unvalidated`: `access_denied` is read off OAuth2 RFC 6749 §4.1.2.1 and `Error getting user email from external provider` out of GoTrue's `api/external.go`, and neither has been seen arriving here. One refused round trip each against the live project - Cancel on Google's consent screen, and a GitHub account with no verified address - with the returned-to URL read off the address bar, would settle both.
