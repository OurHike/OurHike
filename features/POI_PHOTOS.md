...
**Built today:** the Commons source (`pipeline/fetch_poi_images.py`, the `photo_*` properties, the card's credit line) with its images served from our own bucket (#362), ATC's facility photos (`fetch_atc_photos.py`, rung 3), the **private half of "Your own photo"** — capture from the card with a keep-or-discard review (#571), several per place with a sticky choice of which one the card shows (#575), with the archive-honesty behaviour #573 asked for (see "not a photo archive" below) — and the **store and serve half of "Sharing"** (#576): the backend gallery with its caps, the recorded licence and attribution request, and a contributor withdrawal that works, rendered on the card as rung 2 with rung 1 above it (#578). What remains unbuilt of "Sharing" is its two hiker- and moderator-facing surfaces: the share sheet (#577) and pin/takedown (#579), both being designed on their own claim — until the sheet exists, nothing can put a photo into the store an ordinary hiker can reach.
