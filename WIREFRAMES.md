...
**Every sheet shows Light / Standard / Fine, greyed where it has none.** The hiking sheet is published at one measured size and used to render no picker at all; beside a sheet that offers three, an absent control cannot say whether this map has no smaller version or whether the app forgot to ask. The levels are drawn under every tab, disabled where nothing is published behind them, and the note states the one size instead. The same greying carries the case where levels exist but no choice can be taken — bytes are here or on their way, so changing detail means downloading again, and the note says so rather than the control vanishing.
...
2. **Map size** — the whole corridor at Light 64 MB / **Standard 314 MB** (recommended) / Fine 1.18 GB. ⚠ The wireframe copy adds "...or take single sections later, in Downloads" — **drop that clause**, see Known Deviations. **Amended 2026-08-06 ([#298](https://github.com/jaimito-asuntos-gringuenos/OurHike/issues/298)):** this step and the download window it opens onto are two consecutive views of one decision, and they disagreed about what the decision was — one detail level here, a sheet per tab there, with the level applying to only one of those sheets. It now renders the same tabs and the same levels from the same catalog (§4), so the sizes on this screen are the sizes on the next one. What it does *not* carry is the download itself: no progress, no buttons, nothing on the phone to delete. That belongs to the window, one screen later.
