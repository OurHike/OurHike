...
- **R2 · The map never leaves.** Map and step column share the screen — side by side on a desktop, stacked on a phone; Details is read against the route it describes. On a desktop step 3 is the rail beside the map (`desktop.css`, `App.dayHike.test.tsx`); on a phone it is a sheet over it.
