...
def test_fails_on_a_region_tile_missing_from_the_archive(tmp_path, requests_mock, monkeypatch):
    archive = build_archive(tmp_path, requests_mock, monkeypatch)
    # Rebuild with one tile 404ing - the exporter tolerates the hole, and
    # that tolerance is exactly what this gate exists to stop at publish.
    ...
    assert any("missing from the archive" in p and "2/2/1" in p for p in problems)
