...
- **Finer than the grid helps one model and hurts the other.** Correcting NBM's temperature for elevation made tomorrow's high worse (2.7 °F off on average across eight stations became 3.0; 2.8 became 4.5 at Mount Washington), while correcting HRRR's made it the best in the table (2.3). So the first two days' temperature comes from HRRR corrected to each trail point's height and everything else from NBM as published — the maintainer's call — measured over 61 summer days, with winter still to check.
...
- **An hourly publish needs an hourly clock, and GitHub's is not one** — #1346 measured the "hourly" conditions bake at every 3 h 53 m. The maintainer accepted that for weather, so a forecast read in signal is typically 2–4 hours old and says so.
