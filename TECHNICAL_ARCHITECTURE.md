...
2. **Load** — load already-fetched raw data into DuckDB tables *before* any transformation, the "L" in an extract-**load**-transform (ELT) pipeline. *(Design decided 2026-07-29; **built** as `pipeline/load_raw.py`, Phase A, 2026-08-18 — see `pipeline/DBT.md`, whose status line records all four phases. 28 raw tables load as of Phase D, 2026-08-27. Deliberately excludes raster pixel data; the lightweight raster **metadata** table this bullet also promised is the one part still unbuilt — nothing in Phases A–D needed it — which keeps the warehouse itself small per value #8.)*
3. **Transform (dbt)** — turn loaded raw tables into clean, tested, documented staging/intermediate/marts models — SQL by default, Python only for a documented, measured exception. *(Design decided 2026-07-29; **built** — Phases A and B 2026-08-18, Phases C and D 2026-08-27, see `pipeline/DBT.md`. The first vertical slice landed as planned and then widened: 27 staging models across six organizations feed `int_pois_unioned` → `dim_pois`, with 115 data tests and a required CI check. What is still **not** built is the wiring into Export — the dotted edge above — and `DBT.md` names the price of doing it carelessly: `dim_pois` carries `public_use` unapplied, so **a row in `dim_pois` is not a publishable POI**.)*
...
**Which clock each source runs on, and what "only process what changed" can mean below the artifact level, is [pipeline/INCREMENTAL.md](pipeline/INCREMENTAL.md)** (designed 2026-09-09, unbuilt beyond #1311's first delta unit). Two things there bear on this section rather than on that one: the change-detection step 1 describes is already per-source, so running sources on their own intervals is a *scheduling* change and not a fetch change; and the raw bytes those fetchers pull are proposed to live in a **second, private R2 bucket**, never in the public one this page's publish step writes — raw layers carry rows this project has decided never to publish. Revised 2026-09-24 (#1653): a **second private bucket** (the third bucket in all) holds the output of each expensive step under a hash of its inputs, so a step whose inputs did not move is not re-run. It starts as plain files (GeoParquet, JSON) rather than a single DuckDB file or Iceberg tables, and INCREMENTAL.md carries the measured comparison.
...
- **Stack:** React + TypeScript, **MapLibre GL JS** (open-source map renderer, no vendor lock-in) reading **PMTiles** directly in-browser. Two kinds of local data: the background sheet — since 2026-08 a self-built **OSM-schema vector basemap** by default ([pipeline/BASEMAP.md](pipeline/BASEMAP.md), #237), with the clipped USGS raster quads as the full-detail tier rather than the architecture (this bullet used to name the raster as the architecture, #661) — and small **vector GeoJSON** layers (ATC + NYS OPRHP + NYS DEC + OSM + NHD) for the POIs users actually search/filter, loaded directly as MapLibre GeoJSON sources, no vector-tiling step needed at this data volume. *(The parenthesis read "ATC + OSM + NHD" and the noun read "AT-specific POIs" until 2026-08-27; both had been wrong since `export_nearby_poi.py` began shipping NYS OPRHP's and NYS DEC's facilities — [#1097 — *Ship DEC's and OPRHP's POIs — the six types the coverage survey measured, and neither org's water*](https://github.com/OurHike/OurHike/issues/1097).)*
