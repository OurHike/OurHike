...
- **Default: every trail line is one red** — `client/src/lib/blaze.ts`'s `PLAIN_TRAIL_COLOR`, an alias of the palette's Red rather than a new hue, so the closed palette above is exactly as closed as it was. Red light still wins over it, as it wins over the hues.
